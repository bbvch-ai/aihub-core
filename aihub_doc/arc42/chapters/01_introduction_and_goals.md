...
The Swiss AI Hub addresses these barriers by packaging the full AI infrastructure stack as a single deployable product
under the Apache 2.0 license. Organizations get a working system out of the box and retain the ability to inspect,
modify, and self-host every component.

The platform is structured around a tier model that reflects how organizations typically adopt AI:

**Tier 1 (Secure AI access):** A web-based chat interface (OpenWebUI) provides LLM access through a unified gateway
(LiteLLM) that routes to cloud models (Azure OpenAI, Google Gemini, Cohere) or locally hosted models (llama.cpp, vLLM).
...
Microsoft Teams, Slack, and Outlook, via the Azure Bot Framework. The same security policies and governance controls
...
embeddings, and store them in Milvus. Specialized agents built with the SDK can query this organizational knowledge base
to give contextually grounded answers. The Swiss AI Agent Protocol, an event-driven communication standard over NATS,
...
The platform is developed by bbv Software Services, a Swiss software engineering company. The platform infrastructure is
licensed under Apache 2.0 and the SDK under EUPL 1.2, with a commercial license option for proprietary extensions. A
Contributor License Agreement applies to external contributions.
...
| Model-agnostic LLM routing      | LiteLLM provides a unified OpenAI-compatible API to any configured model. Switching providers requires a configuration change, not a code change.                           |
...
| PII detection and anonymization | Presidio intercepts requests before they reach external LLM providers, detecting and redacting personally identifiable information.                                         |
| Per-user cost tracking          | LiteLLM and Langfuse track token consumption and cost per user, per agent, and per trace. Token budgets can be set per user.                                                |
| Full audit trail                | Every agent step, LLM call, retrieval operation, and user interaction is logged as an immutable event in the NATS event stream and persisted to MongoDB.                    |
| Role-based access control       | Hierarchical permissions (OAuth2, LDAP, Azure AD) with wildcard-capable permission strings scoped to specific agents and resources.                                         |
...
| **bbv (platform developer)**                                   | A codebase that supports rapid iteration (monorepo, shared libraries, automated testing). A clear boundary between platform and SDK that allows independent release cycles. Open-source distribution that builds trust in regulated markets.                                                            |
| **Integration partners** (IT service providers)                | A platform they can deploy and operate for their own clients without deep AI expertise. Clear documentation and tooling. Clear licensing terms (Apache 2.0 for the platform, EUPL/commercial dual license for the SDK).                                                                                 |
| **Open-source community**                                      | Permissive licensing (Apache 2.0) for the platform. Transparent development practices (public repository, issue tracker, CI). Contributor License Agreement that preserves the project's ability to evolve its licensing while protecting contributor rights.                                           |
