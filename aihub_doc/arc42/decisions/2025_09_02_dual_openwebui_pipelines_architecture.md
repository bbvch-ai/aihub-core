...
The AI-Hub platform needed to integrate with Open-WebUI to provide users access to both sophisticated AI agents and
simple LLM models through a unified interface. The challenge was that these represent fundamentally different use cases:
...
- **Agent Workflow Visibility**: AI-Hub agents generate rich events (ThoughtEvent, ToolEvent, RetrieverEvent,
...
1. **Event-Based Agent Pipeline** (`aihub_pipeline.py`): Complex event processing to showcase AI-Hub agent capabilities
   with full OpenWebUI feature utilization
...
- **Optimized User Experience**: Agent pipeline showcases full AI-Hub capabilities with rich UI features, model pipeline
  provides straightforward LLM access
