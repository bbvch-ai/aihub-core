...
**Rationale**: Different agents serve different purposes. Preference for detailed explanations in a research agent
shouldn't affect preference for concise responses in a code assistant.
...
**Rationale**: Factual organizational context, projects, and relationships are not agent-specific. When one agent learns
"Project Falcon uses microservices," all agents should understand this for consistent assistance.
...
1. **Metadata Preservation**: Ensure complete Swiss AI Agent Protocol context (thread, display, run) is preserved during
   memory updates for full traceability
2. **Graph Integrity**: Validate entity relationships before creation to prevent orphaned connections in the knowledge
   graph
