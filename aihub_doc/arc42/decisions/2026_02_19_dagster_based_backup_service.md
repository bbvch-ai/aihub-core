...
- A single container runs `dagster-daemon` (background, handles schedules and run monitoring) and `dagster-webserver`
  (foreground, serves UI on port 3000)
- Dagster state (run history, event logs, schedule state) is stored in SQLite at `$DAGSTER_HOME` inside the container
...
- Backups are modeled as a daily-partitioned Dagster multi-asset producing six outputs (postgres, milvus, neo4j,
  clickhouse, valkey, nats), giving a calendar view of backup coverage in the UI
- A `daily_backup_schedule` triggers online backups at 2 AM Europe/Zurich
- Restores are parameterized jobs (`full_restore_job`, `single_service_restore_job`) configured via the Dagster
  Launchpad
