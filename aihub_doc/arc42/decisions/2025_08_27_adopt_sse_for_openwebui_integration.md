...
The AI-Hub platform needed to provide better integration with Open-WebUI, a popular open-source web interface for
conversational AI. Previously, the integration relied on OpenAI-compatible endpoints that streamed simple text chunks,
which had significant limitations in communicating AI-Hub's rich event system (ToolEvent, ThoughtEvent, etc.).
...
AI-Hub's sophisticated event system or Open-WebUI's native streaming capabilities effectively.

The AI-Hub already had a sophisticated event system built around NATS messaging, with rich event types (ThoughtEvent,
ChunkEvent, ToolEvent, HumanInTheLoopRequestEvent, etc.) that needed to be efficiently translated into Open-WebUI's
native data structures for optimal user experience.

## Decision Drivers

- **Rich Event Communication**: OpenAI-compatible endpoints only support text chunks, while AI-Hub has rich events
...
- **Better Event Translation**: AI-Hub's event system needed to be cleanly translated into Open-WebUI's expected data
...
- **Event-Driven Architecture Alignment**: Both AI-Hub and Open-WebUI are built around event-driven patterns, and SSE
  provides a natural bridge between them
...
Create dedicated SSE streaming endpoints in the AI-Hub API:

1. **Agent Event Streaming**: Endpoint pattern `/api/v1/agents/{agent_class}/{agent_id}/{event_name}/stream`
2. **Native AI-Hub Event Format**: Stream events in AI-Hub's native event format rather than trying to adapt them to
   generic formats
...
1. **Event-Driven Processing**: Use Chain of Responsibility pattern for handling different AI-Hub event types
...
3. **Content Block Architecture**: Abstract content blocks with proper inheritance hierarchy (TextBlock, ThinkingBlock,
   ToolBlock)
4. **HTML Serialization**: Convert AI-Hub events into Open-WebUI compatible HTML structures

### Decision 3: Event Translation Layer

Create a comprehensive event translation system:

1. **Event Handler Chain**: Specialized handlers for each AI-Hub event type (ThoughtEventHandler, ChunkEventHandler,
   ToolEventHandler, etc.)
2. **Open-WebUI Event Emission**: Translate AI-Hub events into Open-WebUI's expected event format
...
- **Native Event Support**: AI-Hub's rich event system (thoughts, tools, retrieval) translates cleanly to Open-WebUI's
  interface
...
- **Reduced Connection Management**: No need to manage connection lifecycle, authentication handshakes, or reconnection
  logic
- **Event Fidelity**: Native AI-Hub event format preserves all event metadata and structure
...
- Rich AI-Hub event streaming: Full support for thoughts, tools, retrieval, and human-in-the-loop interactions
