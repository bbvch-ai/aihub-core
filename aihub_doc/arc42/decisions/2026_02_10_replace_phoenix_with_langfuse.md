...
AI-Hub uses an LLM observability platform for end-to-end agent tracing, cost monitoring, and evaluation experiments.
Previously, Arize Phoenix served this role for both tracing (via OpenTelemetry) and evaluations (via a custom
`PhoenixExperimentEvaluator`).

Phoenix is distributed under the **Elastic License 2.0 (ELv2)**, which explicitly prohibits providing the software as
part of a managed or SaaS offering. AI-Hub ships as a turnkey Docker Compose stack that customers deploy as a service —
...
- **Elastic License 2.0 incompatibility**\
  Phoenix's ELv2 prohibits bundling within a SaaS or managed-service offering. AI-Hub's Docker Compose distribution
  model breaches this restriction. Continuing to ship Phoenix exposes the project to legal risk.
...
**Auto-provisioning**: A `LangfuseProvisioner` runs on API startup to register AI-Hub agents and LLM connections in
...
Langfuse UI. A default prompt template maps dataset questions to agent requests.
