...
sicher, dass die KI faktisch fundiert agiert und bei fehlenden Informationen dies transparent kommuniziert.
...
vollständigen Quelldokumenten navigieren.
`UNKLARHEIT IN DER DOKU - BITTE PRÜFEN: Eine explizite Anzeige des Konfidenzgrades oder der Unsicherheit der KI-Antwort als UI-Funktion wird in der Quelldokumentation nicht beschrieben. Der "Context Sufficiency Guard" erfüllt eine ähnliche Funktion, indem er unzureichenden Kontext markiert und so die KI dazu befähigt, ihre Wissensgrenzen transparent zu kommunizieren.`
...
domänenspezifische Optimierungen vorgenommen werden.
...
unvoreingenommene Modellvergleiche.
`UNKLARHEIT IN DER DOKU - BITTE PRÜFEN: Die Quelldokumentation erwähnt keine automatischen Trigger oder Alarme bei schlechtem Feedback auf Systemebene. Sie gibt jedoch an, dass das Feedback-System Organisationen hilft, Bereiche für Verbesserungen zu identifizieren, was manuelle Intervention impliziert.`
...
enthalten repräsentative Testfragen mit bekannten Referenzantworten. Die Qualität der Agentenantworten wird objektiv
durch drei unabhängige **KI-Richter** (Large Language Models) gemessen, die jede Antwort anhand spezifischer
Bewertungsmetriken einstufen. Dies schafft einen revisionssicheren Audit-Trail für die Agenten-Performance und erlaubt
die Verfolgung von Qualitätsänderungen über die Zeit.

### Technische Umsetzung im Swiss AI Hub: Datasets, Experimente und Bewertungsmetriken

Administratoren erstellen im Bewertungsdienst **Datasets** aus Frage-Antwort-Paaren (empfohlen: 20-50 Paare, die
einfache und komplexe Szenarien abdecken). Anschliessend werden **Experimente** ausgeführt, bei denen ein ausgewählter
Agent gegen ein Dataset getestet wird. Drei LLM-basierte **KI-Richter** bewerten jede Agentenantwort (0-5 Sterne) nach:

- **Korrektheit**: Faktische Genauigkeit im Vergleich zur Referenzantwort.
- **Vollständigkeit**: Behandelt alle Aspekte der Anfrage.
- **Prägnanz**: Effiziente und direkte Formulierung.

Die Ergebnisse werden auf einer dedizierten Seite angezeigt, die Gesamtmetriken sowie eine detaillierte Aufschlüsselung
pro Frage inklusive Agentenantwort und Latenz bietet. Niedrige Korrektheitswerte weisen auf Wissenslücken oder
Abrufprobleme hin. Für tiefere Analysen können **Phoenix Tracing** (Entwicklung) und **SigNoz** (Produktion)
herangezogen werden. Letztere Plattform (SigNoz) bietet Dashboards zur Überwachung von Infrastruktur-, Anwendungs- und
KI-Operationen (Modellnutzung, Token-Verbrauch, Kosten pro Operation) und ermöglicht die Konfiguration flexibler
Alarmierungsfunktionen für kritische Dienstausfälle oder Leistungsverschlechterung.
...
Die Plattform legt die technischen Grundlagen, um Aspekte wie Bias und Model-Drift zu adressieren und A/B-Tests zu
ermöglichen, auch wenn dies nicht immer als vollautomatisierte Out-of-the-Box-Funktion bereitgestellt wird. Das
Bewertungs-Framework erlaubt den Vergleich verschiedener Agenten-Konfigurationen oder Prompt-Strategien vor dem
Deployment. Durch die detaillierte Observability können Veränderungen in der Systemleistung oder im Nutzerverhalten als
Indikatoren für potenzielle Qualitätsverschlechterungen dienen.

### Technische Umsetzung im Swiss AI Hub: Bewertungs-Framework als Basis

`UNKLARHEIT IN DER DOKU - BITTE PRÜFEN: Die Quelldokumentation beschreibt keine automatisierte Bias-Erkennung, Fairness-Metriken oder dedizierte Model-Drift-Erkennung als integrierte Funktionen. Ebenso werden keine integrierten A/B-Testfunktionen mit Traffic-Aufteilung in der Produktion angeboten.`
Das **Bewertungs-Framework** des Swiss AI Hub bietet jedoch die grundlegenden Funktionen, die für die **Unterstützung
dieser Aspekte** erweitert werden könnten. Es erlaubt den Vergleich der Qualität verschiedener Agenten (die
unterschiedliche Prompts, Modelle oder Retrieval-Strategien nutzen können) anhand definierter Datasets *vor dem
Deployment*. Dies ermöglicht es, verschiedene KI-Modelle (z.B. GPT-4, Claude, Gemini über LiteLLM-Proxy) oder
Prompt-Konfigurationen systematisch zu evaluieren und die beste Variante auszuwählen. Die umfassende
**OpenTelemetry-Tracing-Grundlage** und die **SigNoz-Dashboards** bieten detaillierte Metriken zu LLM-Nutzung,
Token-Verbrauch und Latenz, die als Indikatoren für Performance-Veränderungen dienen können. Administratoren können
Alarme für diese Metriken konfigurieren, die auf potenzielle qualitative Verschlechterungen hinweisen. Die Behebung von
Bias erfolgt primär durch sorgfältiges **Prompt-Tuning** und gegebenenfalls **Modell-Feinabstimmung** mit qualifizierten
Datensätzen, wobei die Evaluierungs-Tools des AI Hub zur Validierung der Verbesserungen eingesetzt werden können.
...
Darüber hinaus können Umgebungsvariablen für Konfigurationen und Geheimnisse genutzt werden, die ebenfalls versioniert
und über sichere Kanäle verwaltet werden sollten.
`UNKLARHEIT IN DER DOKU - BITTE PRÜFEN: Eine explizite Aussage zur Versionskontrolle von *Prompt-Templates* innerhalb der Plattform oder ein dediziertes Feature-Set für Rollbacks spezifisch auf die *Qualität der Prompts* fehlt. Die Versionierung des Agenten-Codes und der Plattform-Komponenten wird in Kapitel 10 impliziert, aber nicht spezifisch auf Prompt-Management bezogen. Die Konfiguration über Environment Variables wird versioniert, und die Docker Images selbst können versioniert und zurückgesetzt werden.`
