...
gegeben sind. Für Schweizer Unternehmen, die in einem streng regulierten Umfeld agieren und höchste Anforderungen an
...
KI-Entscheidungen den internen und externen Richtlinien entsprechen und jederzeit auditierbar sind.

### Konzepte & Prozesse: Strukturierte Workflows und Ereignis-Trennung

Die Architektur des Swiss AI Hub basiert auf einem konsequenten Workflow-Ansatz. Agenten folgen explizit definierten,
schrittweisen Prozessen, anstatt autonom und unkontrollierbar zu agieren. Jeder dieser Schritte kann die volle Leistung
der KI nutzen, um zu argumentieren und Entscheidungen zu treffen, doch der Gesamtpfad bleibt stets durch den
vordefinierten Workflow gesteuert.
...
eine präzise Kontextualisierung jeder Aktion und damit eine hohe Granularität für Tracing und Debugging.

### Technische Umsetzung im Swiss AI Hub: SAAP und OpenInference
...
externen Systeme: Alle HTTP- und Datenbankoperationen werden automatisch instrumentiert und in den Traces sichtbar
gemacht. Für die visuelle Aufbereitung dieser Informationen während der Entwicklung bietet die Plattform die **Phoenix
UI**, die unter `http://localhost:6006` LLM-spezifische Observability mit Timeline-Ansichten, Token-Nutzung und
Inspektionsmöglichkeiten abgerufener Dokumente bereitstellt.
...
Option, sondern eine zwingende Anforderung. Der Swiss AI Hub stellt sicher, dass Unternehmen diese Anforderungen
...
und DSGVO (Art. 15) umfassend zu unterstützen.
...
Kontext jedes Ereignisses wird über die hierarchischen Scopes (`Thread`, `Display`, `Run`) präzise zugeordnet.
...
Antwort dienten. Dies ermöglicht eine transparente Überprüfung der Faktenbasis und minimiert das Risiko von
"Halluzinationen". Logs sind zentralisiert, strukturiert und über den OpenTelemetry Collector verarbeitbar.
...
Dokumente. Alle Logs, einschliesslich der detaillierten Audit-Protokolle, werden über verschlüsselte Kanäle übertragen
(TLS/HTTPS) und sind durch die rollenbasierte Zugriffskontrolle (RBAC) der Observability-Plattform geschützt, um
Manipulationssicherheit zu gewährleisten. Die Plattform bietet APIs für Benutzerprofile, Konversations-Threads und
Audit-Logs, um Auskunftsrechten nachzukommen.
...
(Metriken, Logs, Traces). Dies gewährleistet, dass die Plattform mit jedem OTLP-kompatiblen Observability-Backend
integriert werden kann, ohne eine Vendor-Lock-in zu riskieren. Dashboards und flexible Alarmierungsfunktionen
ermöglichen eine schnelle Visualisierung und proaktive Benachrichtigung bei kritischen Ereignissen.
...
und Sicherheitsereignisse konfiguriert und an Kanäle wie E-Mail, Slack oder Microsoft Teams weitergeleitet werden. Durch
die OTel-Grundlage können Telemetriedaten auch an alternative OTLP-kompatible Backends wie Grafana, Datadog oder Splunk
exportiert werden, indem lediglich die Collector-Konfiguration angepasst wird.
...
Management der Betriebskosten von KI-Systemen für Unternehmen von grosser Bedeutung. Der Swiss AI Hub bietet
...
Die Kosten von KI-Operationen werden primär durch die **Token-Nutzung** bestimmt. Der Swiss AI Hub verfolgt diese Kosten
für jede Konversation, unabhängig vom LLM-Anbieter (Cloud oder selbst gehostet). Dies ermöglicht eine granulare Analyse,
welche Agenten, Workflows oder sogar einzelne Abfragen die höchsten Kosten verursachen. Zukünftig können Budgets und
Ratenbegrenzungen durch Administratoren festgelegt werden, um Ausgabenlimits und Nutzungsbeschränkungen durchzusetzen.
Optimierungsstrategien umfassen die dynamische Modellauswahl basierend auf der Aufgabenkomplexität (Flaggschiff,
Ausgewogen, Effizient) und die Abwägung zwischen variablen Pay-per-Token-Gebühren von Cloud-APIs und festen
Infrastrukturkosten bei lokal gehosteten Modellen.
...
LLM-Interaktion. Der `cost_reporting_llm()` Context Manager umschliesst das LLM in Agenten-Workflows, um die
Token-Nutzung und die Veröffentlichung von `LLMCostEvent`s zu automatisieren, sobald der Kontext verlassen wird. Diese
`LLMCostEvent`s sind `Display Events` und enthalten Details zu Token-Anzahl und zugehörigen Ausgaben. Diese
Informationen werden im Konversationsverlauf angezeigt und ermöglichen es Administratoren, Ausgaben auf Agenten-,
Benutzer- oder Thread-Ebene zu verfolgen. Obwohl die Funktionen für Budgets und Ratenbegrenzungen bereits existieren,
sind sie standardmässig nicht aktiviert und erfordern eine Umgebungskonfiguration während des Deployments. Die Plattform
unterstützt die detaillierte Nachverfolgung, um teure Queries zu identifizieren und die Modellauswahl für
Kostenoptimierungen anzupassen.
