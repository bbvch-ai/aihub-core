...
Design ermöglicht, ohne die Kernfunktionalität zu beeinträchtigen. Die geräteunabhängige Zugänglichkeit sorgt zudem
dafür, dass Mitarbeitende jederzeit und überall produktiv sein können.
...
über alle Funktionen hinweg. Der Service-Katalog präsentiert Benutzern auf Basis ihrer Rollen und Berechtigungen eine
Reihe spezialisierter Dienste – von der Agenten- und Thread-Verwaltung über Wissensmanagement und Prozessautomatisierung
bis hin zu Evaluationsdiensten. Die nahtlose Navigation zwischen diesen Diensten ermöglicht einen ununterbrochenen
Workflow.
...
um unternehmensspezifische Funktionen erweitert wurde. Die Oberfläche ist direkt in die Swiss AI Hub Suite eingebettet
und über eine vereinheitlichte Navigation zugänglich. Ein konsistentes visuelles Design und dieselben Interaktionsmuster
sind über alle Dienste hinweg etabliert. Die **responsiven Web-Komponenten** und die Möglichkeit der **Installation als
Progressive Web App (PWA)** gewährleisten eine optimale Darstellung und Funktionalität auf Desktops, Tablets und
Mobiltelefonen. Ein **White-Labeling** der Chat-Interfaces und Web-Komponenten ermöglicht die Anpassung an das
Unternehmensdesign (Logos, Farbschemata, Schriftarten), um eine konsistente Markenführung zu sichern.
`UNKLARHEIT IN DER DOKU - BITTE PRÜFEN: Eine explizite Aussage zur WCAG-Konformität (Barrierefreiheit) als fester Bestandteil des Designs fehlt, wird aber als "gewährleistet oder erreicht werden kann" erwähnt. Der "Design-for-All"-Ansatz wird in den Kernaussagen genannt, aber nicht explizit in der Quelldokumentation für die UI aufgeführt.`
...
um präzise und umfassende KI-Antworten zu generieren.
...
**Dokumente direkt in Konversationen hochladen** (PDFs, Office-Dokumente, Textdateien) und Fragen zum Inhalt stellen.
Der Dokumenten-Upload erfolgt intuitiv, typischerweise per Klick auf einen "Mehr"-Button und Auswahl der Dateien. Eine
explizite Erwähnung von Drag-and-Drop oder PDF/A-Unterstützung findet sich in der Quelldokumentation nicht, ist aber bei
modernen UIs oft implizit. Für visuell gestützte KI-Modelle können Benutzer auch **Bilder über Bildschirmfreigabe** in
...
KI-generierte Antworten gering. Der "Black-Box"-Effekt ist eine der grössten Hürden für die Akzeptanz in Unternehmen.
...
Quellenanzeigefeld ist interaktiv und ermöglicht die Überprüfung von Aussagen. Ergänzt wird dies durch detaillierte
**Ausführungsverfolgungen (Observability)**, die Einblicke in die Entscheidungslogik der Agenten geben, sowie durch
**Benutzerfeedback-Mechanismen**, die eine kontinuierliche Verbesserung der Modellleistung ermöglichen.
...
Quellenanzeigefeld an, wenn KI-Antworten auf organisatorisches Wissen verweisen. Benutzer können die spezifischen
Dokumente und Passagen einsehen, die die Antwort beeinflusst haben, und direkt zu den vollständigen Quelldokumenten im
Wissensmanagementdienst navigieren. Dies minimiert Halluzinationen und macht diese transparent.
`UNKLARHEIT IN DER DOKU - BITTE PRÜFEN: Eine explizite Warnung bei externen Links (GDPR) wird in der Quelldokumentation nicht beschrieben. Dies wäre eine zusätzliche Sicherheitsmassnahme.`
...
Dieses Live-Tracing bleibt über Konversationssitzungen hinaus bestehen. Benutzer können zudem **Feedback** (Daumen
hoch/runter) zu KI-Antworten geben, was in ein Elo-basiertes Ranglistensystem fliesst, das die Modellleistung bewertet.
...
Die Versionskontrolle für Dokumente im Wissensmanagement (siehe Kapitel 06) stellt sicher, dass veraltete Informationen
konsistent entfernt und relevante Versionen referenziert werden.
...
APIs für Benutzerprofile, Konversations-Threads und Audit-Logs sind verfügbar, um Auskunftsrechten nachzukommen.
...
Die PII-Anonymisierung mittels Presidio (siehe Kapitel 07) schützt zusätzlich sensible Daten im Fluss.
...
die Dokumentsprache für die Verarbeitung und das Wissensmanagement automatisch erkennen.
...
Ressourcen wie Wissens-Namespaces können Übersetzungen für alle unterstützten Sprachen enthalten. Die **Spracheingabe
unterstützt Schweizerdeutsch**, neben Deutsch und Englisch, und KI-Antworten können auch als Sprache ausgegeben werden.
Zahlen, Daten, Zeiten und Währungen werden gemäss den regionalen Konventionen formatiert. Die Suchfunktionalität
verwendet sprachspezifische Tokenisierung und Stemming, um präzise Ergebnisse in jeder Sprache zu liefern.
Organisationen können zudem benutzerdefinierte Übersetzungen für unternehmensspezifische Begriffe hinzufügen.
