...
This guide provides detailed, step-by-step instructions for setting up the AI-Hub development environment. It is
designed to walk you through installing the necessary tools, configuring your IDE (specifically JetBrains PyCharm and
WebStorm), and getting the codebase up and running locally.

## 🛠️ Install Required & Recommended Tools

::: info Getting Started Before you begin, you need to install several tools. Follow the instructions for your operating
system. :::
...
- **Git**: The version control system for our project.
  ...
  - You can download it [directly](https://git-scm.com/) or install it through your IDE. JetBrains IDEs have excellent
...
- **Python (3.13) via Miniconda**: We use Miniconda to create isolated Python environments.
  ...
  - > **Note**: Miniconda is used only to create the virtual environment and install the base Python version. Package
    > management within the environment is handled by Poetry.
...
- **Docker**: For running our application stack in containers.
  ...
  - **Option 2: Docker Desktop (License)** ::: warning Commercial Licensing Docker Desktop requires a paid license for
    commercial use. Verify your compliance with Docker Desktop's licensing terms. :::

    - [Download here](https://www.docker.com/products/docker-desktop/)
    - **Windows**: For WSL errors, see
      [this solution](https://stackoverflow.com/questions/78879806/docker-desktop-wsl-update-failed)
...
- **Node.js (LTS) through NVM**: We use NVM to manage Node.js versions for frontend development.
  ...
  - Install NVM for [Windows](https://github.com/coreybutler/nvm-windows/releases) or
...
- **Additional Tools**:
  ...
  - **Bot Framework Emulator**: For testing chatbots.
    [Download here](https://github.com/microsoft/BotFramework-Emulator).
...
The `aihub-core` repository is a monorepo containing multiple independent microservices (e.g., `aihub_api`,
`aihub_agents`). The following steps explain the recommended way to set up the project in PyCharm for a smooth
...
1. **Set Up Poetry Environments for Each Microservice**:
   ...
   - For each attached project:
     ...
     - For the **Base interpreter**, select the Python executable from your Miniconda installation (e.g., in
...
1. **Set Up Project Dependencies in PyCharm**:
   ...
   - For each microservice project (`aihub_api`, `aihub_bot`, `aihub_agent`, `aihub_pipeline`), check the box for
     `aihub_lib` to mark it as a dependency.
...
- Once the services are running, explore the `playground` directories within each microservice. These contain scripts
  and notebooks for testing and experimenting with the code in a sandbox environment. This is a great way to understand
  how different components work.

## ⚙️ IDE Configuration and Plugins (Recommended)

To optimize your development workflow, we recommend installing the following plugins and enabling specific settings in
your JetBrains IDE.
...
- **Plugins**:
  ...
  - [SonarLint](https://www.sonarsource.com/products/sonarlint/features/jetbrains/): Real-time code quality and security
    checks.
...
- **Settings**:
  ...
  - **Black Code Formatter**: Enable Black to automatically format your Python code. Find the setting under **Tools >
    Black**.
...
- **EsLint**: Enable this to automatically lint your JavaScript/TypeScript code. You can configure it to run on save
  under **Settings > Languages & Frameworks > JavaScript > Code Quality Tools > ESLint**.
