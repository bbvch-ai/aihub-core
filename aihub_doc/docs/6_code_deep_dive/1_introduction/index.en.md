...
The project is a monorepo using uv workspaces, containing multiple packages ("scopes"), such as `aihub_agent` or
`aihub_api`. A single virtual environment and lockfile are shared across all packages.

::: warning :warning: Important
To install all dependencies for the entire workspace, run the following command from the repository root:

```bash
uv sync --all-packages
```

This installs dependencies for all scopes at once. No per-scope environment activation is needed -- `uv run`
automatically uses the workspace virtual environment.
:::
...
Use the following commands to manage dependencies. Run `uv add` and `uv remove` from the scope directory whose
dependencies you want to change. Do not edit the `pyproject.toml` or `uv.lock` files manually.

- `uv sync --all-packages`: Installs all dependencies defined in `uv.lock`.
...
From the scope directory you worked on, run:
...
Inter-package dependencies are declared in `[tool.uv.sources]` with `{ workspace = true }`, and uv workspaces
automatically resolve them from local directories during development. For deployment, packages reference `aihub_lib` via
its Git URL in the `pyproject.toml` file, with versioning handled by Git tags.
:::
