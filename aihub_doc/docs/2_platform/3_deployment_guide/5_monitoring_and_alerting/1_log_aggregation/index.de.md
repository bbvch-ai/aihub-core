...
Die Observability-Architektur des Swiss AI-Hub basiert auf **OpenTelemetry**, wodurch Sie Logs, Metriken und Traces in
externe Systeme für zentralisierte Verwaltung, Langzeitarchivierung und erweiterte Analysen exportieren können. Obwohl
...
SigNoz bietet einen vollständig verwalteten Cloud-Dienst mit regionalen Endpunkten (EU, US, IN). Der AI-Hub ist
...
Für Produktions-Deployments wird die **Selbst-Haltung von SigNoz auf einer dedizierten VM** aus mehreren Gründen
dringend empfohlen:
...
SigNoz kann mit Docker Compose auf einer separaten VM mit entsprechenden Ressourcen (4+ CPU-Kerne, 8+ GB RAM, 100+ GB
Speicher) bereitgestellt werden. Der OTel Collector des AI-Hub wird dann so konfiguriert, dass er auf den selbst
gehosteten Endpunkt anstelle von SigNoz Cloud zeigt.
...
OpenInference Traces werden **sowohl** an Phoenix (lokales, spezialisiertes LLM-Debugging) als auch an SigNoz (Cloud,
