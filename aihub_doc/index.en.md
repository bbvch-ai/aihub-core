...
[Slack & Teams Integrations](docs/2_platform/15_slack_teams_integrations/).
:::
...
secure network perimeter. Review our comprehensive [Security features](docs/2_platform/18_security/).
:::
...
[OpenAI-Compatible API](docs/2_platform/16_api/1_openai_compatible_api/) allowing integration with many existing tools,
...
systems can trigger agents via the platform's [Agent Interaction API](docs/2_platform/16_api/2_agent_interaction_api/);
automated Data Pipelines can sync knowledge from sources like SharePoint; and standard protocols are supported for
custom connections. See [External Integrations](docs/2_platform/20_external_integrations/).
:::
