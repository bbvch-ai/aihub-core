...
layout: home
title: Swiss AI Hub
source_sha: 52004dd3c2110a37cb6d727792ee7cdda8943e42a8418f3272b59162125d54a5
...
features:
  - title: KI in 30 Minuten deployen
    details: Ein Befehl startet alles. LLM-Gateway, Vektordatenbanken, Chat-Interface, Authentifizierung, Monitoring. Vorkonfigurierte Agenten funktionieren sofort. Keine Cloud-Konten, kein komplexes Setup.
  ...
  - title: Sehen Sie genau, was die KI tut
    details: Jede Entscheidung ist nachvollziehbar. Jeder Workflow-Schritt ist sichtbar. Jede Kostenposition wird verfolgt. Wenn die KI eine Antwort gibt, können Sie sehen, warum. Vertrauen durch Transparenz, nicht durch Versprechungen.
  - title: Entwickeln Sie ohne Infrastruktur-Probleme
    details: Authentifizierung, Deployment, Monitoring, Skalierung sind bereits gelöst. Schreiben Sie die Geschäftslogik Ihres Agenten, die Plattform kümmert sich um alles andere. Konzentrieren Sie sich auf das, was Sie einzigartig macht.
  - title: Funktioniert mit Ihrer bestehenden Infrastruktur
  ...
  - title: Wachsen Sie mit dem Ökosystem
    details: Jede Schweizer Organisation, die die Plattform nutzt, stärkt sie. Teilen Sie gemeinsame Agenten, halten Sie strategische privat. Kooperieren Sie bei der Infrastruktur, konkurrieren Sie bei der Innovation.
---
...
::: details Wie kann unsere Schweizer Organisation KI einsetzen und gleichzeitig alle Daten On-Premise halten?
...
::: details Wie können wir KI nutzen und gleichzeitig Schweizer Datenschutzgesetzen wie dem FADP (revDSG) entsprechen?
Die Gewährleistung der **Schweizer Datensouveränität** ist ein Kernprinzip der Swiss AI Hub Community. Da *Sie* die
Plattform deployen, entscheiden Sie, *wo* sie läuft – entweder auf Ihren eigenen Servern in der Schweiz oder in
...
erfüllen. Lesen Sie mehr über unser Engagement unter
...
Observability-Funktionen. Jeder Schritt eines KI-Agenten ist sichtbar, Entscheidungen werden mit Kontext protokolliert
und Kosten werden verfolgt. Tools wie Phoenix ermöglichen die Nachverfolgung jeder Interaktion, sodass Sie jederzeit
...
Vektordatenbanken für KI), Datenverarbeitungspipelines und Benutzeroberflächen sofort nach der Installation. Dies löst
viele gängige **Herausforderungen in der produktiven KI** von Anfang an. Erfahren Sie mehr darüber im
...
einsatzbereit sind und eine schnelle Wertschöpfung ohne komplexe Cloud-Konfigurationen ermöglichen. Beginnen Sie mit der
[Schnellstart-Anleitung](/de/docs/2_platform/1_quick_start/).
:::
...
Kollaborationstools, die sie täglich nutzen, was den Workflow verbessert. Details finden Sie unter
...
konfigurierten KI-Modellen dient. Sie können den Modellzugriff zentral verwalten, Anfragen basierend auf Richtlinien
...
den **zuverlässigen Unternehmenseinsatz** erforderlich sind. Siehe
[Unsere Lösung](/de/docs/1_vision_and_positioning/1_introduction/2_our_solution/).
:::

::: details Wie können wir KI-Agenten sicher Fragen auf Basis unserer internen Unternehmensdokumente (wie PDFs oder Word-Dateien) beantworten lassen?
Der Swiss AI Hub enthält ein sicheres **Retrieval-Augmented Generation (RAG) System**. Sie konfigurieren automatisierte
[Data Pipelines](/de/docs/2_platform/6_pipelines/), um Dokumente aus Ihren Quellen (wie SharePoint) aufzunehmen. Diese
...
::: details Verschiedene Teams in unserer Organisation verwenden unterschiedliche KI-Tools, wodurch Silos entstehen. Wie können wir einen vereinheitlichten, gesteuerten KI-Ansatz schaffen?
...
[OpenAI-kompatible API](/de/docs/2_platform/16_api/1_openai_compatible_api/), die die Integration mit vielen bestehenden
...
Der Swiss AI Hub verwendet **Data Pipelines**, die mit dem robusten Orchestrator Dagster erstellt wurden. Diese
Pipelines automatisieren den gesamten Workflow: Verbindung zu Ihren Datenquellen, intelligentes Parsen verschiedener
Dateiformate, Erstellung semantischer Chunks, Generierung von Vektor-Embeddings und deren Indizierung in Ihrem
Vektorspeicher (wie Milvus). Details finden Sie im [Pipelines-Abschnitt](/de/docs/2_platform/6_pipelines/).
:::
...
Ja. Wenn Sie den Swiss AI Hub **On-Premise** deployen und so konfigurieren, dass er ausschliesslich **selbst gehostete
...
::: details Wie stellen wir sicher, dass die KI-Agenten vertrauenswürdige Antworten liefern und nicht einfach "halluzinieren" oder sich Dinge ausdenken?
Vertrauen ist von grösster Bedeutung. Swiss AI Hub **Agenten** sind so konzipiert, dass sie expliziten, definierten
Workflows folgen. Sie verwenden hauptsächlich **Retrieval-Augmented Generation (RAG)**, was bedeutet, dass ihre
Antworten auf Informationen basieren, die aus *Ihren* verifizierten Unternehmensdokumenten stammen. Agenten **zitieren
...
[Agenten](/de/docs/2_platform/5_agents/).
:::

::: details Können KI-Agenten auf dieser Plattform bei komplexen Aufgaben menschliche Experten um Hilfe oder Genehmigung bitten?
...
Genehmigung an einen benannten menschlichen Experten sendet (zum Beispiel über eine Slack-Nachricht) und dann nahtlos
seine Arbeit fortsetzt, sobald der Mensch antwortet. Entdecken Sie die
...
[Agent Interaction API](/de/docs/2_platform/16_api/2_agent_interaction_api/) der Plattform triggern; automatisierte Data
Pipelines können Wissen aus Quellen wie SharePoint synchronisieren; und Standardprotokolle werden für benutzerdefinierte
Verbindungen unterstützt. Siehe [Externe Integrationen](/de/docs/2_platform/20_external_integrations/).
:::
...
ermöglicht, Anstrengungen beim Aufbau und der Verbesserung der grundlegenden KI-Infrastruktur zu bündeln. Jeder
profitiert von gemeinsamen Fortschritten, wodurch sich einzelne Organisationen auf die Entwicklung einzigartiger
KI-Anwendungen konzentrieren können, die ihren spezifischen Bedürfnissen entsprechen, was die gesamten KI-Fähigkeiten
der Schweiz stärkt. Lesen Sie mehr über
...
Die Swiss AI Hub Community hat die Plattform für eine **vereinfachte Deployment und Verwaltung** konzipiert. Die
Installation erfolgt mit einem einzigen Befehl, und kritische Infrastrukturherausforderungen wie Authentifizierung,
Monitoring und Skalierungskonfiguration sind sofort gelöst. Dies reduziert die Betriebskomplexität erheblich im
