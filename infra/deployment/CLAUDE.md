...
## Sysadmin Subdomain (BSL Plane)

The system-administration plane (`packages/sysadmin-api`, `packages/sysadmin-web` — BUSL-1.1) is served on its **own
subdomain** `sysadmin.${DOMAIN}`, separate from the main app on `${DOMAIN}`. Routing is **path-split** via Traefik:
`sysadmin-web` (static SPA) serves `/` and `sysadmin-api` serves `/api/v1` (priority 6000, above the web catch-alls,
mirroring the main `api`/`web` priority pattern). Both share the **same Keycloak realm and `aihub-frontend` client** as
the main app, so the realm SSO cookie spans `${DOMAIN}` and `sysadmin.${DOMAIN}` — the realm import (see Keycloak
Operator Notes) adds the `sysadmin.${DOMAIN}` redirect URIs / web origins. Let's Encrypt SANs must include
...
To add another BSL plane in future, follow the same recipe: hyphenated `compose-config.yml` key (== image == package
dir), a service block on the `sysadmin.${DOMAIN}` host (or a new subdomain) with bracket-access image tags, the
