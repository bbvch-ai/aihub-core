...
- **`OTEL_DEPLOYMENT_ENVIRONMENT` / `OTEL_HOST_NAME` are the one sanctioned `${VAR:-default}`.** The collector's
  ...
  build — which exits the collector. Nothing `depends_on` the collector, so an unset var would silently drop all
  telemetry rather than just one attribute. Values are written per-VM into `.env` by `aihub-playbook`'s `env.j2`; the
  stage-name fallback exists so a non-Ansible deploy still starts. Do not "fix" it by moving the default to `.env.prod`.
