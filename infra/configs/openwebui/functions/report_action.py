...
class Action:
    ...
    def _resolve_model(
        ...
        OpenWebUI injects `__model__` for actions, but its shape has moved between versions and the message itself
    ...
    async def action(
        ...
        try:
            # json.dumps rather than f-string interpolation: a model identifier is third-party
            # data and must not be able to terminate this script.
            code = f"""
