...
# Permission prefix constants
AIHUB_ADMIN_PREFIX = "aihub.admin."
AIHUB_USER_PREFIX = "aihub.user."
...
class AccessChecker:
    ...
    def __init__(self, user_access_rules: list[str]):
        ...
        self.admin_access_rules = {r for r in self.valid_access_rules if r.startswith(AIHUB_ADMIN_PREFIX)}
        self.user_access_rules = {r for r in self.valid_access_rules if r.startswith(AIHUB_USER_PREFIX)}
    ...
    def validate_user_access_rule(access_rule: str) -> bool:
        ...
        if not access_rule.startswith((AIHUB_USER_PREFIX, AIHUB_ADMIN_PREFIX)):
            return False
    ...
    def validate_permission_template(template: str):
        ...
        if not template.startswith((AIHUB_USER_PREFIX, AIHUB_ADMIN_PREFIX)):
            raise ValueError(
                f"Invalid permission template: Must start with '{AIHUB_USER_PREFIX}' or '{AIHUB_ADMIN_PREFIX}'. Got: {template}"
            )
    ...
    def access_level(self, permission_template: str) -> AccessLevel:
        ...
        admin_perm_to_check = permission_template.replace(AIHUB_USER_PREFIX, AIHUB_ADMIN_PREFIX, 1)
        ...
        if permission_template.startswith(AIHUB_USER_PREFIX):
    ...
    def access_level_for_agent(self, agent_class: str, agent_id: str) -> AccessLevel:
        ...
        return self.access_level(f"{AIHUB_USER_PREFIX}agent.{agent_class}.{agent_id}")
    ...
    def has_access_to_agent_class(self, agent_class: str) -> bool:
        """Convenience method to check access level for a specific agent."""
        return self.access_level(f"{AIHUB_USER_PREFIX}agent.{agent_class}.?*") != AccessLevel.ACCESS_DENIED
    ...
    def access_level_for_process(self, process_class: str, process_id: str) -> AccessLevel:
        ...
        return self.access_level(f"{AIHUB_USER_PREFIX}process.{process_class}.{process_id}")
    ...
    def has_access_to_process_class(self, process_class: str) -> bool:
        """Convenience method to check access level for a specific process."""
        return self.access_level(f"{AIHUB_USER_PREFIX}process.{process_class}.?*") != AccessLevel.ACCESS_DENIED
    ...
    def access_level_for_service(self, service_name) -> AccessLevel:
        ...
        return self.access_level(f"{AIHUB_USER_PREFIX}service.{service_name}")
