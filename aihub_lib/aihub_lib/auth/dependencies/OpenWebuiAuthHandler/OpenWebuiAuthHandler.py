...
from aihub_lib.persistence.user.UserEntity import UserEntity # Still used for fallback/initial token check
...
class OpenWebuiAuthHandler(BearerAuthHandler):
    ...
    async def __call__(
        ...
        logger.debug(f"OpenWebuiAuthHandler: OpenWebUI headers found: Name='{open_webui_user_name}', Email='{open_webui_user_email}'.")
        ...
        logger.info(f"Comparing hashes E-Mail: {computed_open_webui_user_email_hashed} vs. {open_webui_user_email_hashed}")

        is_hash_valid = computed_open_webui_name_hashed == open_webui_user_name_hashed and computed_open_webui_user_email_hashed == open_webui_user_email_hashed
        ...
        graph_user_profile_by_email = await self.graph_service.get_user_by_email(open_webui_user_email, access_token_for_graph)

        if not graph_user_profile_by_email or not graph_user_profile_by_email.get("id"):
            logger.warning(f"OpenWebuiAuthHandler: Could not resolve OpenWebUI email '{open_webui_user_email}' to a user in Graph.")
        ...
        logger.debug(f"OpenWebuiAuthHandler: Fetching Graph details for OID {user_oid_to_use} in context of app {self.app_client_id_for_roles}.")
        user_app_details = await self.graph_service.get_user_details_for_app_context(
            user_oid=user_oid_to_use,
        ...
        if not profile_from_graph:
            logger.warning(f"OpenWebuiAuthHandler: User OID {user_oid_to_use} not found via Graph, falling back to OpenWebUI headers.")
        ...
        logger.debug(f"OpenWebuiAuthHandler: Roles from Graph for OID {user_oid_to_use} (app context {self.app_client_id_for_roles}): {roles_to_use}")
        ...
        logger.info(f"OpenWebuiAuthHandler: Authenticated user OID {final_user.oid}, Name: {final_user.name}, Roles: {final_user.roles}")
        return final_user
