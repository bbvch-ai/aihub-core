...
def superuser_handler() -> SuperuserAuthHandler:
    ...
    return SuperuserAuthHandler(identity_provider=SuperuserIdentityProvider())


@pytest.fixture
def result_container() -> dict:
...
@async_test
async def authenticate_with_token(superuser_handler: SuperuserAuthHandler, token: str, result_container: dict) -> None:
...
@async_test
async def authenticate_with_empty_token(superuser_handler: SuperuserAuthHandler, result_container: dict) -> None:
...
def check_user_name(result_container: dict, expected_name: str) -> None:
...
def check_user_email(result_container: dict, expected_email: str) -> None:
    """Check that the returned user has the expected email."""
    user = result_container.get("user")
    assert user is not None, "No user returned by SuperuserAuthHandler"
    assert user.email == expected_email, f'Expected user email "{expected_email}", got "{user.email}"'


@then(parsers.parse('the returned user should have oid "{expected_oid}"'))
def check_user_oid(result_container: dict, expected_oid: str) -> None:
...
def check_user_role(result_container: dict, expected_role: str) -> None:
    """Check that the returned user has the expected role."""
    user = result_container.get("user")
    assert user is not None, "No user returned by SuperuserAuthHandler"
    assert expected_role in user.roles, f'Expected role "{expected_role}" not found in {user.roles}'


@then(parsers.parse("an HTTPException with status code {status_code:d} should be raised"))
def check_http_exception(result_container: dict, status_code: int) -> None:
...
def check_exception_detail(result_container: dict, expected_detail: str) -> None:
...
class TestSuperuserAuthHandler:
    ...
    @pytest.mark.asyncio
    async def test_inheritance_from_bearer_auth_handler(self):
        """Test that SuperuserAuthHandler properly inherits from BearerAuthHandler."""
        handler = SuperuserAuthHandler(identity_provider=SuperuserIdentityProvider())

        # Check that it's an instance of BearerAuthHandler
