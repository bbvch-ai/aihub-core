...
from aihub_lib.infrastructure.azure.blob_storage.BlobStorageConfig import BlobStorageConfig
...
class BlobStorageAccess:
    ...
    def __new__(cls):
        if cls._instance is None:
            cls._instance = super(BlobStorageAccess, cls).__new__(cls)
    ...
    def _initialize(self):
        ...
        self._storage_service_name = BlobStorageConfig().BLOB_STORAGE_NAME or f"{self._app}st{self._region}datalake"
        self._service_endpoint = (
            BlobStorageConfig().BLOB_STORAGE_ENDPOINT or f"https://{self._storage_service_name}.blob.core.windows.net"
        )
    ...
    def get_url_signing_secret(self) -> str:
        return BlobStorageConfig().URL_SIGNING_SECRET
