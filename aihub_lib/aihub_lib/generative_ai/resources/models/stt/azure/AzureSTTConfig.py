...
class AzureSTTParameter(ResourceParameter):
    ...
    prompt: Annotated[Optional[str], Field(
        description="An optional text prompt to guide the model's style or continue from a previous audio segment."
    )] = None

    response_format: Annotated[Optional[Literal["json", "text", "srt", "verbose_json", "vtt"]], Field(
        description="The format of the output transcription."
    )] = "json"
    ...
    timestamp_granularities: Annotated[Optional[List[Literal["word", "segment"]]], Field(
...
class AzureOpenaiSTTConfig(STTConfig, AzureOpenaiResourceConfig):
    ...
    """
    # Because this field uses a default factory, it’s best to leave the assignment using Field().
