...
**Tech Stack & Paradigms**: **NATS** pub-sub (nats-py) with typed Pydantic events. Hierarchical event model: Control (workflow) vs Display (observability). **MongoEngine** ODM for MongoDB (FerretDB). **LlamaIndex Core** + extensions (vector stores: Milvus/Azure AI Search, storage: MongoDB docstore, readers: file, postprocessors: Cohere rerank, embeddings/LLMs: OpenAI-like). **Redis** client (v5.2.1) for Valkey state storage. **Azure SDK** suite (speech, cosmos, search, storage, document intelligence, cognitive services). **OpenTelemetry** full instrumentation suite (SDK, API, exporters, semantic conventions) + **OpenInference** for LLM tracing. **Arize Phoenix** for observability. **FastAPI** for REST. **python-i18n** for translations. **Pydantic + pydantic-settings**. **OpenAI SDK**, **google-genai**, **transformers** (HuggingFace). **Docling** for document processing. **boto3** for AWS. **cachetools** for TTL caching. **colorlog** for logging. **fsspec** for filesystem abstraction. pytest + pytest-bdd + pytest-mock for testing.
