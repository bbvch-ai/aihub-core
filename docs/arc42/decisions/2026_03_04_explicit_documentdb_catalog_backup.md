...
Swiss AI Hub's FerretDB instance runs on a PostgreSQL backend with the
...
**Maintenance**: The table and sequence lists are constants in
`packages/backup/swiss_ai_hub/backup/services/postgres.py` with an inline comment containing the exact SQL query to run
