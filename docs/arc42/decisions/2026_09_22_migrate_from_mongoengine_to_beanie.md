...
becomes either a manual operator step or an `if old_shape:` branch in code — which is also why four entities run with
`strict: False`.
...
- **It makes the schema-drift problem tractable**\
  Four entities run with `strict: False` because old document shapes drift and nothing can remove them deliberately. A
  working migration framework is what permits normalising that drift and then tightening the models — which is #1152's
...
Every one of these was found by a check that first appeared to pass. All five failure modes are **silent** — no
exception, no warning, success reported — so none of them would be caught by a reviewer reading a diff.

1. **Never set `use_transaction=True`.** FerretDB 2.5 does not implement `commitTransaction`
   (`code 59, CommandNotFound`). With the flag on, the migration's writes **land**, the commit then fails, the run
   raises, and `migrations_log` is left **empty** — data changed with nothing recording it, so a retry re-applies the
   migration to already-migrated documents. It is the flag an operator reaches for *because they want safety*, and on
   FerretDB it buys the appearance of atomicity with none of the substance. Enforced in CI, not left to a comment.

2. **Every entity ported from a `strict: False` document declares `extra="allow"`**, until its drift has been normalised
   by a migration. Without it, a single `replace()` deletes every undeclared field. The other write paths survive only
   because `save()` uses `$set`-style semantics — an implementation property no test of ours asserts.
...
**A migration that fails partway is safe**, which makes "refuse to start on migration failure" a viable policy: a
migration that raised mid-run wrote **nothing at all**. Beanie collects its `replace_many` calls as coroutines and
awaits none of them until the whole collection is transformed (`beanie/migrations/controllers/iterative.py:93-127`), so
the database is left in its pre-migration state and the deploy can be rolled back. This does **not** extend to a failure
inside that final `asyncio.gather`, nor is it verified beyond the 10 000-document default `batch_size`.
...
- **Five silent failure modes to defend against.** CI checks and a routing registry are mitigations, not guarantees; a
...
| 14  | Does it survive partial failure, concurrent replicas and many databases? | **1 PASS, 2 FAIL**          | A failed migration writes **nothing**. But four concurrent replicas **double-applied** every document while all reporting success (rule 4), and concurrent multi-database migration corrupted (rule 5). |
