...
generated by jambo, which cannot carry `AgentSchedule`'s cron and timezone validators, so a malformed schedule can
reach storage through the generic config route. The scheduler therefore treats an unparseable stored schedule as a
