...
- **Guards (all `403`).** `auto_sync` databases (SharePoint/OneDrive-fed) are refused — their source pipeline would just
  ...
  > refused too, and `DatabaseDTO.deletable` means "the database itself or one of its namespaces may be deleted", gating
  > both affordances in the UI. Single documents are unaffected — their removal is published to the owning pipeline,
  > which the frozen images still consume — so they stay deletable. The `deleting` flag is a work queue only for
