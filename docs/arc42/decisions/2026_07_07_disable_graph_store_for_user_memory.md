...
- Keep the change reversible and measurable, and avoid a silent cross-agent regression.

## Decision

Make the mem0 graph store **per-memory-type**:

- **User memory: graph OFF** (opt-out, reversible via `MEM0_ENABLE_USER_MEMORY_GRAPH`, default `True` to preserve
  current behavior during rollout). `AgentMemory` builds a graph-less mem0 service for user memory; mem0 then skips the
  entire graph branch (3 LLM calls + graph embeddings).
...
**Cross-agent replacement (replaces graph purpose 1):** when the user-memory graph is off, user-memory *reads*
...
Flag-off dev run (~28 stored memories): Ministral LLM calls **5 → 2**, embedding calls **~120 → 13**, save wall-clock
...
- Reversible per environment via `MEM0_ENABLE_USER_MEMORY_GRAPH`.
...
**Open question (resolve before broad rollout / flipping the flag on in production):** production graph-usage stats —
what fraction of users interact with 2+ agents, and what fraction of user-memory retrievals return non-empty relations.
If cross-agent user memory and the relations block are rarely exercised, the negatives above are immaterial; if common,
validate the vector-based cross-agent replacement covers the need.

## Related

- Supersedes the user-memory graph portion of `2025_12_18_adopt_mem0_for_agent_memory` (organization memory portion
  stands).
- Neo4j data is retained and the flag is reversible; a future graph-store replacement (Neo4j is flagged as outdated) is
