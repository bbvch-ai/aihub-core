...
- JSON-schema mode returns JSON wrapped in a ` ```json ` fence, prose instead of JSON, truncated JSON, or a multi-block
  response the parser rejects;
...
The team has **temporarily disabled** the two most affected conversational features —
`#1535` (conversation title + follow-up generation) and `#1536` (the per-agent self-awareness meta-question steps, see
ADR `2026_06_04_self_awareness_as_explicit_per_agent_steps` and `2026_06_18_conversation_metadata_as_explicit_per_agent_steps`).
However, the **underlying structured-output fragility still affects live agents**: the `FewShotAgent`'s suitability
check calls `agent_description_guard`, and the `RAGAgent` calls `few_shot_guard` and `context_sufficient_guard` — all of
which still crash on Kimi/Qwen.
...
**2. Fail open on relevance guards.** `agent_description_guard`, `few_shot_guard`, and `context_sufficient_guard` catch
the residual parse/validation error after retries and return a safe default that **does not block the pipeline**
(`success=True` → accept the request / treat context as sufficient). These are relevance/quality guards, not security
controls; the security-relevant `sensitive_info_guard` is deliberately **not** given a fallback. Guard failures are
logged with the exception **type only** (never the raw exception text, which can echo the user query or retrieved
context).

**3. Plain-text token classification for meta-question detection.** `detect_meta_question` no longer uses
`astructured_predict`. It sends a purpose-built English prompt asking for exactly one token
(`NORMAL` / `META_IDENTITY` / `META_CAPABILITIES` / `META_BEHAVIOR`), calls `achat`, and parses the last token; an
unrecognized response degrades to "not a meta question". Reasoning is disabled for this call
(`extra_body={"chat_template_kwargs": {"thinking": false}}`) since the classification is trivial — cutting latency from
~4-5s to sub-second with no loss of accuracy.
...
- **Relevance guards fail open.** When a reasoning model cannot return a parseable verdict, the guard accepts the
  request rather than rejecting it. This is a deliberate, documented choice: these guards gate topical relevance and
  retrieval sufficiency (not a security boundary), and failing closed would reject *every* request on a model that
  fails structured output frequently — making the agent unusable. The decision is scoped to relevance guards only; the
  security-relevant `sensitive_info_guard` retains fail-closed behavior. When guards degrade, the agent still answers
  using only its already-configured knowledge.
- **Guards are best-effort on reasoning models.** Their decisions (suitability, context-sufficiency) effectively
  pass-through whenever the model returns unparseable output, reducing answer-quality filtering for those models.
...
- **The real fix is provider-side.** These are mitigations around a flaky managed endpoint. An instruct model
  (gemma-4 / Ministral / Apertus) makes all structured features work natively; reasoning models remain best-effort until
  Infomaniak corrects their tool-call/reasoning/structured-output handling.
