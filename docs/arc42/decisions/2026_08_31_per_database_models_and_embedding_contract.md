...
- `2026_09_04_ingestors_announce_their_configuration_form` supersedes decision 1. The two columns became settings the
  pipeline declares for itself, held in one configuration object on the database. Decisions 3 and 5 stand: a
  collection's dimension is still derived from the embedding model, and a model that cannot state its width is still
  refused when the database is created.
