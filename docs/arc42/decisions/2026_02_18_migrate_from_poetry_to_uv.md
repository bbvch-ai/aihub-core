...
Swiss AI Hub is a Python monorepo with six packages (`aihub_lib`, `aihub_agent`, `aihub_api`, `aihub_bot`,
`aihub_pipeline`, `aihub_process`) that share a common library (`aihub_lib`). Previously, each package used Poetry as
its package manager with independent `pyproject.toml` and `poetry.lock` files.
