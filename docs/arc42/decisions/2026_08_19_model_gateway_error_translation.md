...
2. **The trace was not marked.** `opentelemetry-instrumentation-asgi` has no exception branch in
   `OpenTelemetryMiddleware.__call__` — only `try`/`finally` for metrics. An exception escaping the app ends the server
   span with status `UNSET` and no `http.status_code`, so the request never appeared in any error-filtered view.
...
(`packages/api/swiss_ai_hub/api/exceptions/model_gateway_error_handler.py`), registered in `ApiRunner._get_api_app` for
...
**3 — Every such failure marks the current span `ERROR`.** This deliberately departs from the OTel HTTP convention,
...
- One registration covers every endpoint that talks to a model — chat, embeddings, images, STT, TTS — including the
  dynamically registered agent endpoints.
