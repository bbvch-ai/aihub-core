...
- **Customer deployments**:
  ...
  - `aihub-Ig*s` - **Gen 2 pilot** (Ansible Pull + Infomaniak), pre-production; deploy-only, core app images `:latest`
    (unpinned)
...
- **Kubernetes deployment (Gen 3, mới nổi)**:
  - `aihub-k8s` - Terraform (Azure AKS + Stoney OpenStack Magnum) + 2 Helm chart (`aihub-common`, `aihub-tenant`) cho
    **multi-tenancy theo namespace** và scale-out theo chiều ngang. Cả hai chart khai báo `appVersion: "0.1.0"` và pull
    image qua `${CORE_VERSION:-latest}` — chart **không** pin aihub-core version cụ thể; phiên bản core đang chạy là giá
    trị `CORE_VERSION` lúc apply. Các tenant `tenant1`, `jointcreate`, `postgres-test` chỉ là test/sample - chưa có
    customer production migrate sang.
...
| aihub-Ig\*s                 | core images `:latest` (unpinned)                                                                                   | Customer **Gen 2 pilot** (pre-production) - Ansible Pull + Infomaniak OpenStack; deploy-only (stock core images); stack core-aligned nhất (FerretDB + Valkey + Docling); RAG chỉ thị an ninh nội bộ ("IGS Guisan") |
...
- Cả 5 Gen 1 customer (B*D/C*C/W*P/Dem*scope/F*H) chạy SDK version khác nhau, tất cả đều cũ hơn core. Không có policy ép
  upgrade. Khoảng drift: 11 → 104 minor (F*H ở v0.186.0 là drift lớn nhất, đi sau core 104 minor).
...
- **Gen 3 (`aihub-k8s`) đã đóng một phần gap "No K8s migration path"** nêu ở §3.1 Item #20: Helm chart, Terraform cho 2
  cloud (Azure AKS + Stoney OpenStack Magnum), CloudNativePG + Keycloak Operator + cert-manager + NGINX Ingress đã được
  commit. **Tuy nhiên**: chưa có customer production nào trên path này (mới chỉ có tenant `tenant1`, `jointcreate`,
  `postgres-test` ở dạng test); Stoney Magnum có limitation đã ghi nhận là `node_count` không update được sau khi tạo
  cluster; Milvus mặc định chạy standalone (cluster scale-out mode có document nhưng optional); cách Keycloak Operator
  watch cross-namespace được chính tài liệu gọi là "community workaround, not a first-class Keycloak support statement";
  **và chart không pin aihub-core version cụ thể** — chỉ pull image tag mà `CORE_VERSION` resolve tới (xem proposed
  `adr_040`).
...
flowchart TB
    ...
    IGS["aihub-Ig*s (Gen 2 pilot · pre-prod)<br/>deploy-only · core images @ :latest<br/>FerretDB · Valkey · Docling · Milvus<br/>core agents + bot · eval via Langfuse<br/>RAG chỉ thị an ninh nội bộ (DE)<br/>LLM: Azure OpenAI + Swiss LLM Cloud"]
...
| aihub-Ig\*s           | **Pilot / pre-production**          | core images `:latest` (unpinned) | - (deploy-only; core agents + bot) | **Gen 2** - Infomaniak OpenStack + Ansible Pull (customer Gen 2 đầu tiên) | Chỉ thị an ninh / bảo vệ dữ liệu nội bộ (ICT-Sicherheitsweisung, KI-Weisung) - tiếng Đức | Azure OpenAI + Swiss LLM Cloud (cả hai wired; routing không có trong repo) - **partial / chưa verify**               | Azure AD / Entra ID (OIDC)                      | Qua Gen 2 Restic → Swift (verify) | Không (README rỗng) | Không (chỉ eval harness - Langfuse `Citation Quality`)      |
...
| aihub-F\*H            | Production (commit gần nhất 4/2026) | v0.186.0 (sau 104)               | 3A / 2P / 1 API / 1 bot            | **Gen 1** - Azure (Pulumi đã commit: 10 deploy units)                     | Azure Data Lake Storage (TARDOC / TARMED: handbook + positions)                          | Azure OpenAI Switzerland North (`*-openai-sui`) + Azure AI Search (không phải Milvus) - **sovereignty Switzerland**  | Azure AD (AUTH_AZURE_AD\_\*)                    |    Không (không có trong repo)    |        Không        | Minimal (5 test + 5 BDD)                                    |
...
Section này highlight các items cần ưu tiên address để chuẩn bị go-live, group theo scope (Core / B*D / C*C / Dem*scope
...
| 15  | No supply chain security (SBOM/signing/scan)                                                                                                                                                                                                                                                                                                                                                                      |     HIGH     | syft/CycloneDX (SBOM) + cosign (image signing) + trivy (vuln scan) trong CI. Verified: hiện chỉ có Dependabot — chưa có SBOM/signing/scan. Phần process (SAST/DAST/CVSS) ở #34 / proposed `adr_052`                                                                                                                                                                                                                                                                                    |
...
| 18  | No alerting infrastructure formal                                                                                                                                                                                                                                                                                                                                                                                 |     HIGH     | **Đã giảm thiểu một phần**: pipeline OTLP→SigNoz Cloud đã wired, nhưng **không có alert rule as-code, không AlertManager/on-call routing trong repo**; SigNoz là sink Cloud (EU) bên ngoài, không self-hosted trong core. Vẫn cần: alert rules as-code + PagerDuty/OpsGenie on-call + per-service severity rules (xem proposed `adr_050`, `adr_032`)                                                                                                                                   |
| 19  | No business metrics + SLI/SLO formal                                                                                                                                                                                                                                                                                                                                                                              |     HIGH     | **Đã giảm thiểu một phần**: collector có pipeline traces/logs/metrics, nhưng **core (API/agents/pipelines/dispatcher) KHÔNG emit metric nào** (không `MeterProvider` trong `packages/core`; chỉ OpenWebUI/LiteLLM tự phát); **bot không có OTEL** (trace đứt tại bot); host metrics/journald chỉ có qua Gen 2 playbook (vắng ở cả 5 Gen 1 customer). Vẫn cần: business metrics export (agent_runs, HITL escalations, RAG latency) + bot OTEL + formal SLI/SLO (xem proposed `adr_050`) |
...
| 24  | OSS dependency churn, EOL & lock-in (AI stack thay đổi nhanh) — phụ thuộc nhiều lib/hệ thống bên ngoài; khi số lượng tăng, các chủ đề security/maintenance/**End-of-Life** nổi lên mạnh hơn; ngay cả OSS cũng có thể lock-in (proprietary functions, protocols)                                                                                                                                                   |     HIGH     | Hexagonal Ports & Adapters cho 6 layer + contract tests; **cộng** Renovate auto-update + **EoL tracking** + **continuous component-health monitoring** (thủ công/tự động) + eval gate khi swap (xem proposed `adr_043`, `adr_024`)                                                                                                                                                                                                                                                     |
| 25  | Workflow architecture Process vs Agentic chưa quyết định                                                                                                                                                                                                                                                                                                                                                          |     HIGH     | Strategic decision: Option A (activate hybrid Process+Agentic với routing criteria) hoặc Option B (deprecate process clean + migration guide); **marketing architecture diagram (.drawio) cũng đang vẽ process engine** — gỡ ra hoặc align với quyết định                                                                                                                                                                                                                              |
...
| 31  | Marketing architecture diagram quảng bá **Meltano** — không tồn tại trong codebase (`docs/media/architecture/architecture.drawio` có node `meltano`; pipeline thực tế là Dagster + rclone/SharePoint connectors; chính review chỉ nhắc Meltano như *đối thủ* ở #22)                                                                                                                                               |     HIGH     | Gỡ Meltano khỏi marketing diagram hoặc thêm thành roadmap item thật; mở rộng doc-code drift detection sang cả marketing diagrams (false-claim family, gắn #12 và #22)                                                                                                                                                                                                                                                                                                                  |
| 32  | **Chưa có quy trình evaluate accuracy/quality & benchmark performance** (platform-wide) — vấn đề chất lượng chỉ lộ ra khi khách hàng phàn nàn                                                                                                                                                                                                                                                                     |     HIGH     | Quy trình eval chuẩn (golden datasets + LLM-as-judge metrics trong CI) + performance-benchmark gate mỗi release; promote IGS `Citation Quality` + F\*H eval framework thành core capability. **NFR gốc đằng sau §3.6 #12 (F\*H) và §3.5 #11 (W\*P)** (xem proposed `adr_051`, `adr_044`, `adr_046`)                                                                                                                                                                                    |
| 33  | **Supported use cases chưa được document chính thức** — không có câu trả lời "X có support không?" cho sales/audit                                                                                                                                                                                                                                                                                                |     HIGH     | ADR chính thức với ma trận Full/Partial/Out-of-scope; reference từ marketing + pre-sales. Đã nêu ở §1 Weaknesses / §5 "AI use case scope undefined" (xem proposed `adr_037`)                                                                                                                                                                                                                                                                                                           |
| 34  | **Chưa có security testing định kỳ / DevSecOps process** — verified: CI chỉ có Dependabot; không SAST/DAST, không OWASP dependency-check, không CVSS triage, không pentest định kỳ, threat model (§17) không có chu kỳ refresh                                                                                                                                                                                    |     HIGH     | DevSecOps process: SAST (CodeQL/semgrep/bandit) + DAST (OWASP ZAP) trong CI; OWASP Dependency-Check + pip-audit/trivy; CVSS triage feed vào SLA #30; refresh threat-model định kỳ + pentest định kỳ; CycloneDX SBOM (đóng #15) (xem proposed `adr_052`)                                                                                                                                                                                                                                |
| 35  | **Thiếu định nghĩa quality attributes / NFR scenarios để quyết định architecture** — quyết định theo functional; target non-functional bị dựng lại sau khi đã làm                                                                                                                                                                                                                                                 |     HIGH     | Viết arc42 ch10 quality scenarios làm **architecture gate**; sắp xếp tenant-isolation (`adr_002`) + DB migration framework (`adr_003`) **sớm**. Cụ thể: multi-tenancy bị trễ (#3 chưa isolated) → retrofit muộn buộc **data migration** tenant, ảnh hưởng lớn tới khách hàng (C\*C §3.3 #18). NFR phải dẫn dắt architecture, không phải theo sau (xem proposed `adr_053`)                                                                                                              |
| 36  | **Code execution không an toàn (Jupyter)** — Python do user nhập chạy trong jupyter-lab **dùng chung** trên network `backend` (token auth, workspace dùng chung); verified không `cap_drop`/`read_only`/`security_opt`/`pids_limit`/resource limits, không isolate per-user; reach được backend peers (LiteLLM giữ LLM keys). Marketing quảng bá "code execution **sandbox**" nhưng hardening chỉ ở mức container |     HIGH     | Sandbox ephemeral per-session (gVisor/Kata hoặc nsjail) + drop caps + read-only FS + no-new-privileges + cpu/mem/pids quotas + network khoá riêng (không reach backend, không egress) + workspace per-user; sửa lại claim "sandbox" (xem proposed `adr_054`, gắn #12)                                                                                                                                                                                                                  |
| 37  | **Quyền agents / truy cập bus chưa scoped** — NATS dùng 1 token chung **không có subject-level ACL** → mọi agent/bot/service pub/sub được `agent.>` / `process.>` của mọi tenant & thread (đúng §4.0 V3); agents giữ credential DB/S3 rộng; không per-agent least-privilege; không per-tenant tool auth (§13.2)                                                                                                   |     HIGH     | NATS per-account/user permissions theo subject gồm tenant segment (`aihub.tenant.{id}.*`); credential least-privilege cho từng agent; per-tenant tool authorization (xem `adr_002`, `adr_047`)                                                                                                                                                                                                                                                                                         |
| 38  | **Event bus (NATS/JetStream) không được monitor** — endpoint `http: 8222` có nhưng **không được scrape**; không prometheus-nats-exporter, không JetStream metrics (consumer lag, redelivery, stream bytes), không alert → poison message (#8) / consumer kẹt là vô hình                                                                                                                                           |     HIGH     | Scrape metrics 8222/JetStream vào SigNoz (prometheus-nats-exporter hoặc OTLP); alert consumer lag, redelivery, DLQ depth, stream storage; dashboards (gộp vào `adr_050` + #8)                                                                                                                                                                                                                                                                                                          |
...
| 7   | 4 agent variants deploy (persona / multi_personas × public / private)                                                                                  |     HIGH     | Document lý do split public/private trong ADR; verify 4 instance chạy cùng code hoặc merge thành 1 binary với config flag; giảm operational surface                                                  |
...
| 12  | **Khách hàng không dùng / không chạy platform (no use, no run)** — adoption gần như zero; chi phí ops/hosting vẫn chạy mà không tạo giá trị, không có production validation cho upgrade |   **HIGH**   | Làm rõ ý định khách hàng với account management (tiếp tục / tạm dừng / sunset); nếu tiếp tục: xử lý blocker hiệu năng #11 + onboarding push; nếu không: decommission có kiểm soát để cắt chi phí ops. Khi chưa dùng, ROI của upgrade SDK #4 chưa rõ — sắp thứ tự cho hợp lý |
...
| #   | Item                                                                                                                                                                                                                           |   Severity   | Recommendation actions                                                                                                                                                                                                                                                                                                                                                                                                       |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :----------: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | SDK drift 104 minor versions (v0.186.0 vs v0.290.4) - **lớn nhất trong tất cả customers**; upgrade lên latest được đánh giá **rất phức tạp**                                                                                   | **CRITICAL** | Lập multi-step SDK upgrade plan + security delta audit (104 minor = 10+ tháng patches bị miss); upgrade từng bước v0.186 → v0.220 → v0.260 → v0.290; CI gate chặn drift > N minor. Nếu upgrade-in-place không khả thi (104 minor + monkey-patch #2 + divergence Mongo/Redis/Phoenix #5), cân nhắc **rebuild 3 agents from scratch** trên thế hệ core hiện tại (cùng quyết định remediate-vs-rebuild như Dem\*scope §3.4 #12) |
...
Evidence base: `docker-compose.latest.yml` (generated), `Makefile`, `eval/` (Langfuse eval framework), và
`secrets/igs.yml.vault` trong repo `aihub-igs` (HEAD `8eb4237`, 2026). **Trạng thái: pre-production / pilot.**
**Customer đầu tiên dùng pattern Gen 2** (Ansible-Vault config repo → Ansible Pull trên Infomaniak OpenStack).
**Deploy-only** (không có custom agents/pipelines/API; dùng stock core images). Use case: trợ lý RAG nội bộ về **chỉ thị
an ninh thông tin & bảo vệ dữ liệu** ("IGS Guisan", tiếng Đức). Xem file per-customer `c4/igs.md`.

| #   | Item                                                                                                                                            |   Severity   | Recommendation actions                                                                                                                                                                |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------- | :----------: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Bot ship `DANGEROUS_DEV_ONLY_AUTH_FAKE_*` (name/email/oid/roles) trong compose **production**                                                   | **CRITICAL** | Chỉ cho phép fake-auth ở stage `dev`; core hard-guard chặn path này ở non-dev; xoá `BOT_AUTH_FAKE_*` khỏi vault; CI gate. Xác nhận vault live không populate (xem proposed `adr_048`) |
| 2   | Core **app images pin `:latest`** (api/web/bot/agents/pipelines) — drift vô hạn, không reproducible                                             |   **HIGH**   | Pin tag `CORE_VERSION` cụ thể; fail-fast nếu unset; CI gate chặn drift. Cùng họ với `adr_040` (chart pin) và `adr_001` (SDK versioning)                                               |
| 3   | Sovereignty mixed / chưa verify — LiteLLM wired cả Azure OpenAI **và** Swiss LLM Cloud (+ Cohere/Gemini/HF); routing không có trong repo        |   **HIGH**   | Xác nhận routing thực tế trong `litellm-config`; document vị thế (partial) sovereignty bằng ADR; gắn với Core sovereignty path (Option A/B/C, `adr_000`)                              |
| 4   | Observability drift — compose chạy **Phoenix v10.0.4** (pre-Langfuse) + OTEL→SigNoz Cloud, nhưng eval docs ghi **Langfuse** là system of record |   **HIGH**   | Hợp nhất về 1 backend (Langfuse, theo ADR `2026_02_10`); thêm vào compose/playbook tracked; quyết định sovereignty SigNoz region (xem proposed `adr_049`)                             |
| 5   | `CORS_ALLOW_ORIGIN: "*"` ở OpenWebUI (có `# TODO: Make this more secure`)                                                                       |     HIGH     | Giới hạn CORS về origin frontend đã biết; bỏ wildcard trước khi cutover production                                                                                                    |
| 6   | Off-site backup không thấy trong repo (kỳ vọng qua Gen 2 `os_backups` Restic→Swift)                                                             |     HIGH     | Verify role backup Gen 2 đang active cho IGS; document RTO/RPO + restore drill; theo 3-2-1 (xem `adr_030`)                                                                            |
| 7   | Không có own arc42 + ADRs; `README.md` rỗng                                                                                                     |     HIGH     | arc42 tối thiểu (context + deployment + crosscutting) + ADRs cho: Gen 2 pilot, chọn Docling parser, vị thế sovereignty, Phoenix→Langfuse, pin `:latest`                               |
| 8   | Không có unit/integration test (chỉ có Langfuse eval harness)                                                                                   |     HIGH     | Thêm post-deploy smoke tests (health endpoints, OAuth round-trip, LiteLLM ping, OpenWebUI login); giữ eval harness làm answer-quality gate                                            |

> **Điểm mạnh (so với các customer khác):** stack **core-aligned nhất** — FerretDB + Valkey (không phải Mongo + Redis),
> parser **Docling** (khớp `adr_042`), Milvus v2.6.7 + LiteLLM v1.80.5; có **eval framework** hoạt động (custom
> `Citation Quality` judge + dataset `igs_guisan`); đã wire sẵn **Swiss LLM Cloud** (ý định sovereign-LLM); và là
> **customer Gen 2 đầu tiên** (Ansible Pull, auto-reconcile, secrets mã hoá bằng vault).
...
W*P / F*H) cộng **1 Gen 2 pre-production pilot** (Ig*s); priority items theo từng customer được liệt kê chi tiết tại
**§3.4 (Dem\*scope)**, **§3.5 (W\*P)**, **§3.6 (F\*H)**, và **§3.7 (Ig\*s)**. Trong 2 ma trận bên dưới, cột `Ig*s` được
tóm tắt là *pilot → xem §3.7* và [`c4/igs.md`](c4/igs.md) (findings được liệt kê ở đó thay vì lặp lại từng cell);
**Balmer-E\*** vẫn TBD chờ thông tin từ team.
...
- _Concern_:
  ...
  - Deep imports trong `agents/chat_agent/chat_agent/ChatAgent.py` đến
    `swiss_ai_hub.core.generative_ai.{chat_history,guards}` + `swiss_ai_hub.core.i18n.locale_handler` (xem proposed
    `adr_038`)
...
- _Concern_:
  - Drift 44 minor version (v0.246.4\* vs core v0.290.4) — 4.5+ tháng patches bị bỏ (\*SDK pin không có trong repo
    `pyproject.toml`; số liệu carry over từ snapshot trước)
...
- ADRs trả lời 9 design questions: stack divergence (Mongo/Redis), hash partition (1000 partition trên `persona_id`),
  split public/private 4-variant, vị trí sovereignty (Azure SUI + local vLLM), backup MinIO cùng VM, migration Phoenix →
  Langfuse, IaC approach (commit Pulumi hoặc chọn Terraform), test strategy, agent-config evolution
...
- ADRs trả lời 6 design questions: TLS key trong git (rotation + history rewrite), manual VM deployment, identity
  provider (Azure AD / Entra), LLM region + sovereignty, lý do không có own code, chiến lược backup
...
- ADRs trả lời 9 design questions: Azure AI Search vs Milvus, monkey-patch GPT-5 (workaround + removal path), thiết kế
  routing 3-agent (handbook + rules + routing), ingestion TARDOC/TARMED data, lựa chọn MS Bot Framework, identity (Azure
  AD), Pulumi state SPOF, lý do evaluation framework, BITL events (DignityCheck / RecognitionCheck)

### 5.7. aihub-Ig\*s

Gen 2 pre-production pilot, deploy-only. Concerns theo format `Concern → Direction` (go-live items ở §3.7; container
view ở [`c4/igs.md`](c4/igs.md)).

**Bot dev-auth trong compose production**

- _Concern_: service `bot` set `DANGEROUS_DEV_ONLY_AUTH_FAKE_NAME/EMAIL/OID/ROLES` trong `docker-compose.latest.yml`;
  một giá trị được populate là biến Teams bot thành endpoint không xác thực, spoof được role.
- _Direction_: chỉ cho phép fake-auth ở stage `dev`; core hard-guard chặn ở non-dev; xoá `BOT_AUTH_FAKE_*` khỏi vault;
  CI gate. Xem proposed `adr_048`.

**Core images không pin (`:latest`)**

- _Concern_: app images (api/web/bot/agents/pipelines) pull `:latest` → drift vô hạn, deploy không reproducible.
- _Direction_: pin tag `CORE_VERSION` cụ thể, fail-fast nếu unset, CI drift gate (cùng họ `adr_040`, `adr_001`).

**Observability drift (Phoenix vs Langfuse)**

- _Concern_: compose chạy Phoenix v10.0.4 (pre-Langfuse) + OTEL→SigNoz Cloud, nhưng `eval/README.md` ghi Langfuse
  (`langfuse.igs.ai-agents.ch`) là system of record.
- _Direction_: hợp nhất về 1 backend (Langfuse, theo ADR `2026_02_10`); thêm vào compose/playbook tracked; quyết định
  sovereignty SigNoz region. Xem proposed `adr_049`.

**Sovereignty mixed / chưa verify**

- _Concern_: LiteLLM wired cả Azure OpenAI **và** Swiss LLM Cloud (+ Cohere/Gemini/HF); routing thực tế không có trong
  repo (`litellm-config` do playbook Gen 2 cung cấp).
- _Direction_: xác nhận + document routing và vị thế (partial) sovereignty; gắn với Core sovereignty path (`adr_000`).

**CORS wildcard + thiếu docs/tests + verify backup**

- _Concern_: `CORS_ALLOW_ORIGIN: "*"` ở OpenWebUI; `README.md` rỗng, không có own arc42/ADRs; không có unit/integration
  test (chỉ Langfuse eval harness); off-site backup không thấy trong repo.
- _Direction_: giới hạn CORS về origin đã biết; arc42 tối thiểu + ADRs; post-deploy smoke tests; verify role Gen 2
  `os_backups` Restic→Swift đang active cho IGS (`adr_030`).

**Documentation deliverables**: arc42 (context + deployment + crosscutting) + ADRs cho Gen 2 pilot, chọn Docling parser,
vị thế sovereignty, Phoenix→Langfuse, và pin `:latest`. Điểm mạnh cần giữ: stack core-aligned nhất (FerretDB + Valkey +
Docling), Swiss LLM Cloud đã wired, eval harness `Citation Quality` hoạt động.

### 5.8. Other customer projects (placeholders pending input)

Các customer còn lại chưa có thông tin. Mỗi customer sẽ có §5 subsection riêng (tương tự §5.2-§5.7) khi có chi tiết.
...
- **Audit security delta** từ phiên bản pin của từng customer (B*D v0.279.2, C*C v0.274.3, W*P v0.255.6, Dem*scope
  v0.246.4\*, F\*H v0.186.0) → core hiện tại v0.290.4, force-upgrade customers nếu có security patches (\*Demoscope SDK
  pin chưa verify được từ repo, xem footnote ở §Phiên bản các thành phần)
- **Dem\*scope remediate-vs-rebuild**: quyết định upgrade pin rất cũ tại chỗ (agent crash khi khởi động) hay rebuild
  trên core generation hiện tại; và **chính thức chấp nhận rủi ro backup / gia hạn key do khách hàng sở hữu** (RACI
  sign-off)
- **F\*H answer quality**: phê duyệt re-design RAG/vector cho dữ liệu cấu trúc và quyết định AI-Search-vs-Milvus
  (`adr_039`, `adr_044`)
...
- **Component replaceability / continuous-update strategy**: ports & adapters cho các building block có thể swap
  (document parser, vector store, OCR — LLM đã provider-agnostic qua LiteLLM) + Renovate + eval-gated upgrades +
  fallback có tên cho lib commercial/EOL. Tổng quát hoá case MinerU→Docling (`adr_042`, `adr_043`)
- **Giảm upgrade pain mỗi customer**: mô hình single-tenant-per-deployment khiến mỗi lần upgrade customer là bespoke và
  đắt — multi-tenant data layer + SDK versioning policy chính thức là fix mang tính cấu trúc
...
- **Design/analysis gate trước implementation**: yêu cầu một design artefact ngắn — đặc biệt vector-DB
  chunking/schema/index tuning + eval plan — trước khi code. Khoảng trống quy trình này là root cause của các vấn đề
  chất lượng & performance ở F\*H/Dem\*scope/W\*P (`adr_044`)
- **Load-test baselines**: thiết lập baseline cho từng project + core (Locust) và chạy định kỳ; tiền đề cho SLI/SLO và
  để chẩn đoán complaint performance của W\*P (`adr_046`)
...
- **OpenStack Magnum** (Container Infra; dùng trên Stoney cloud): https://docs.openstack.org/magnum/latest/
...
- **Azure AI Search** (F\*H vector backend; thay thế Milvus): https://learn.microsoft.com/en-us/azure/search/
...
- **vLLM** (LLM serving high-throughput; stack local của Dem\*scope): https://docs.vllm.ai/en/latest/
- **LlamaIndex** (RAG framework; F\*H monkey-patch để hỗ trợ GPT-5): https://docs.llamaindex.ai/
...
[^1]: SDK pin của Demoscope không có trong `aihub-demoscope/pyproject.toml`; không có git dependency `swiss-ai-hub-*` và
    docker-compose image không tag version. Số `v0.246.4` được carry over từ snapshot review trước, chờ xác nhận
