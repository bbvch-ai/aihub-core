...
Optional model for user-memory extraction, found inside the **User Memory** section next to the storage toggle. When a
conversation ends, the platform reads it back to distil durable facts about the user and to reconcile them against what
it already knows — dropping duplicates, replacing what has been superseded. That work is short and mechanical, so a
smaller model is usually enough, and it is worth choosing one: extraction is several model calls per stored
