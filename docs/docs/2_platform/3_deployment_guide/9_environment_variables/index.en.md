...
| `LITE_LLM_PROXY_PUBLIC_URL` | `LiteLLMProxySettings.PUBLIC_URL` | `None` | `api` | Internet-reachable LiteLLM URL. Only consumed when registering the Langfuse evaluator LLM connection: Langfuse resolves the baseURL from its own container and refuses private addresses. All in-cluster traffic uses BASE_URL. |
