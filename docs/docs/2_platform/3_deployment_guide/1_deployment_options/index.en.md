...
The Swiss AI Hub can be deployed as a single isolated instance for one organization, or as multiple isolated instances
that optionally share backend LLM resources.

::: info Multi-tenancy vs Multi-instancing
This chapter describes **multi-instancing** (multiple isolated Swiss AI Hub instances). For **multi-tenancy** (multiple
...
A single instance deployment runs a complete, self-contained Swiss AI Hub instance. The organization gets dedicated
...
The Swiss AI Hub can be hosted in three ways depending on organizational requirements.

### On-premise (bring your own server)

You run the Swiss AI Hub on your own servers in your data center.
...
You run the Swiss AI Hub in your own cloud environment (Swiss cloud provider, Azure, AWS, GCP).
...
bbv hosts and manages the Swiss AI Hub for you on Swiss-based cloud infrastructure.
...
You access the Swiss AI Hub through a web interface and APIs. bbv provides SLAs for uptime and support. Less operational
...
Even a misconfiguration of the Swiss AI Hub cannot cause data leakage between instances. Admins from one instance cannot
...
**Multi-instancing** (this chapter) means running multiple completely isolated Swiss AI Hub installations. Each has
separate databases, vector stores, and application servers. Even a misconfiguration cannot cause data leakage between
instances. Use this when you need absolute isolation (e.g., different legal entities, highly sensitive departments).

**Multi-tenancy** ([chapter 15](../../16_multi_tenancy/)) means creating organizational boundaries within a single Swiss
...
- [Core Components](../../../1_vision_and_positioning/3_core_components/) - Swiss AI Hub architecture
