...
(PII) in Benutzerfragen, bevor diese externe LLM-Anbieter erreichen.
...
Presidio läuft als Guardrail in der LiteLLM-Proxy-Schicht. Es scannt Benutzerfragen nach PII-Mustern, bevor Anfragen an
Sprachmodelle gesendet werden.

Zwei Anonymisierungsmodi:

Der Maskierungsmodus ersetzt erkannte PII durch Platzhalter wie `[PERSON]` oder `[EMAIL_ADDRESS]`. Die Frage „Können Sie
diesen Vertrag für John Smith überprüfen?“ wird zu „Können Sie diesen Vertrag für [PERSON] überprüfen?“, bevor sie das
LLM erreicht. Das Modell kann den Kontext weiterhin verstehen und eine nützliche Antwort generieren.

Der Blockierungsmodus lehnt die gesamte Anfrage ab, wenn bestimmte PII-Typen auftreten. Verwenden Sie dies für
hochsensible Daten wie Kreditkartennummern. Der Benutzer erhält eine Fehlermeldung anstelle einer Antwort.
...
IP-Adressen sowie Daten oder Orte, die Personen identifizieren könnten. Die Erkennung verwendet Musterabgleich, reguläre
Ausdrücke und Modelle zur Erkennung benannter Entitäten. Das System unterstützt mehrere Sprachen, darunter Englisch,
...
pro Deployment basierend auf den Anforderungen an die Datensensibilität.
...
Presidio gilt für alle Anfragen, wenn es aktiviert ist. Verwenden Sie den Blockierungsmodus sparsam – nur für
hochsensible PII-Typen wie Kreditkarten, bei denen eine strikte Ablehnung und keine Maskierung erforderlich ist.
