...
::: tip SeaweedFS Filer
In production, the SeaweedFS Filer web UI is accessible at `datalake.${DOMAIN}` (OAuth2 protected, requires the
`AIHubSysAdmin` realm role). In development mode, it's available at `http://localhost:8889` for browsing uploaded files
and debugging storage.
:::
