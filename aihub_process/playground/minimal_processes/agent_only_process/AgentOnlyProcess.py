...
class AgentOnlyProcess(AgenticProcess):
    ...
    @process_step()
    def start_with_output_from_agent_a(
        self,
        ...
        return AgentBWorkRequest(
            start_event=AgentBStartEvent(payload=work_from_agent_a.agent_event.payload)
        )

    @process_step()
    def end_with_output_from_agent_b(
        self,
