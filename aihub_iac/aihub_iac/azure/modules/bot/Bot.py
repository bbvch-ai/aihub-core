...
from aihub_iac.azure.modules.bot.BotSettings import BotSettings
...
class Bot(pulumi.ComponentResource):
    ...
    def __init__(self, stack: str, name: str, config: BotSettings, opts: Optional[pulumi.ResourceOptions] = None):
        ...
        self.webapp_creator = WebAppCreator(
            ...
            self.config.resource_group,
            self.config.location,
            BotSettings().BOT_APP_SERVICE_PLAN_NAME,
        )
    ...
    def _create_webapp(self):
        ...
        app_settings = [
            ...
            *self._get_oauth_env(),
            ...
            web.NameValuePairArgs(name="NAME", value=self.config.bot_anonym_name),
            web.NameValuePairArgs(name="EMAIL", value=self.config.bot_anonym_email),
            web.NameValuePairArgs(name="ROLES", value=self.config.bot_anonym_roles),
            web.NameValuePairArgs(name="OID", value=self.config.bot_anonym_oid),
    ...
    def _get_base_env(self) -> List[web.NameValuePairArgs]:
        ...
        return [
            web.NameValuePairArgs(name="AZURE_SUBSCRIPTION_ID", value=ProjectSettings().ARM_SUBSCRIPTION_ID),
            web.NameValuePairArgs(name="REGION_SHORT", value=ProjectSettings().LOCATION_SHORT),
            web.NameValuePairArgs(name="APP_NAME", value=ProjectSettings().APP_NAME),
        ]

    def _get_registry_env(self) -> List[web.NameValuePairArgs]:
        """Get environment variables for the docker registry"""
        return [
            web.NameValuePairArgs(name="DOCKER_REGISTRY_SERVER_URL", value=RegistrySettings().REGISTRY_URL),
            web.NameValuePairArgs(name="DOCKER_REGISTRY_SERVER_USERNAME", value=RegistrySettings().REGISTRY_USER),
            web.NameValuePairArgs(name="DOCKER_REGISTRY_SERVER_PASSWORD", value=RegistrySettings().REGISTRY_PAT),
        ]

    def _get_oauth_env(self) -> List[web.NameValuePairArgs]:
        """Get environment variables for the oAuth"""
        return [
            web.NameValuePairArgs(name="CLIENT_ID", value=OAuthSettings().CLIENT_ID),
            web.NameValuePairArgs(name="TENANT_ID", value=OAuthSettings().TENANT_ID),
            web.NameValuePairArgs(name="AUTHORITY_URL", value=OAuthSettings().AUTHORITY_URL),
        ]
