...
class HitlHandler:
    ...
    def __init__(self):
        ...
        self.conversation_to_hitl_request_mapping: Dict[str, HumanInTheLoopRequestEvent] = {}

    async def handle_display_event(self, event: DisplayEvent, _: AgentTopic):
        if event.is_hitl_request_event:
            self._handle_hitl_request(cast(event, HumanInTheLoopRequestEvent))
        else:
            return

    def _handle_hitl_request(self, event: HumanInTheLoopRequestEvent):
        ...
        self.conversation_to_hitl_request_mapping[slack_channel_id] = event
        ...
        adapter.continue_conversation(
            ...
            callback=self._hitl_callback(event),
        )

    def _hitl_callback(self, event: HumanInTheLoopRequestEvent) -> Callable[[TurnContext], None]:
        return lambda turn_context: turn_context.send_activity(event.question)

    @staticmethod
    def use_hitl_handler(request: Request) -> "HitlHandler":
        return request.app.state.hitl_handler
