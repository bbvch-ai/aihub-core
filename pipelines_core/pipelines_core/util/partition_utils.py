...
def replace_partition_keys(
    context: OpExecutionContext, partition_name: str, keys: List[str]
):
    ...
    for old_partition_key in old_partition_keys:
        context.instance.delete_dynamic_partition(
            partitions_def_name=partition_name, partition_key=old_partition_key
        )
...
def create_dynamic_partition(
    ...
    return DynamicPartitionsDefinition(
        name=f"{customer_name}_{namespace_name}_{partitions_name}"
    )
