...
class DocumentChangedEventHandler(EventHandler):
    """Handler for document/file change events from AI coding agents"""

    async def can_handle(
        self, event: Annotated[dict[str, Any], "Event to check"]
    ) -> Annotated[bool, "True if DocumentChangedEvent"]:
        return "DocumentChangedEvent" in event.get("_parent_event_names", [])

    async def handle(
        self,
        event: Annotated[dict[str, Any], "Document changed event"],
        context: Annotated[EventContext, "Processing context"],
    ) -> Annotated[bool, "Always returns True"]:
        path = event.get("path", "unknown")
        operation = event.get("operation", "changed")
        content_preview = event.get("content_preview", "")

        # Emit status for file operation
        await context.emitter(
            {
                "type": "status",
                "data": {
        ...
        # Add visual indicator to content stream
        emoji = "📝" if operation == "modified" else "✨" if operation == "created" else "📄"
        file_message = f"\n\n{emoji} **File {operation.capitalize()}:** `{path}`\n"

        if content_preview:
            file_message += f"```\n{content_preview}\n```\n"

        context.state_manager.start_text_block(file_message)
...
class ToolOutputEventHandler(EventHandler):
    """Handler for successful tool execution events"""

    async def can_handle(
        self, event: Annotated[dict[str, Any], "Event to check"]
    ) -> Annotated[bool, "True if ToolOutputEvent"]:
        return "ToolOutputEvent" in event.get("_parent_event_names", [])

    async def handle(
        self,
        event: Annotated[dict[str, Any], "Tool output event"],
        context: Annotated[EventContext, "Processing context"],
    ) -> Annotated[bool, "Always returns True"]:
        tool_name = event.get("name", "Unknown Tool")
        title = event.get("title", tool_name)
        ...
        # Optionally add output preview to content if it's short
        if output and len(output) < 500:
            output_message = f"\n\n> **{title}:** {output[:200]}\n"
            context.state_manager.start_text_block(output_message)
            await context.emitter(
                {
                    "type": "replace",
                    "data": {"content": context.state_manager.serialize_to_html()},
...
class ToolErrorEventHandler(EventHandler):
    """Handler for failed tool execution events"""

    async def can_handle(
        self, event: Annotated[dict[str, Any], "Event to check"]
    ) -> Annotated[bool, "True if ToolErrorEvent"]:
        return "ToolErrorEvent" in event.get("_parent_event_names", [])

    async def handle(
        self,
        event: Annotated[dict[str, Any], "Tool error event"],
        context: Annotated[EventContext, "Processing context"],
    ) -> Annotated[bool, "Always returns True"]:
        tool_name = event.get("name", "Unknown Tool")
        title = event.get("title", tool_name)
        ...
        await context.emitter(
            {
                ...
                "data": {
                    "action": None,
                    "description": f"❌ {title} failed{duration_str}",
                    "done": True,
                    "error": True,
                },
            }
        ...
        error_message = f"\n\n> [!CAUTION]\n> **{title} failed:** {error}\n"
        context.state_manager.start_text_block(error_message)
...
class EventProcessorFactory:
    ...
    def create_chain() -> Annotated[EventHandler, "First handler in chain"]:
        ...
        handlers = [
            ...
            ChunkEventHandler(),
            ToolEventHandler(),
            ToolOutputEventHandler(),  # New: Handle successful tool execution
            ToolErrorEventHandler(),  # New: Handle failed tool execution
            DocumentChangedEventHandler(),  # New: Handle file changes from AI coding agents
