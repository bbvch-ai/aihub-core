...
class ConfigAuthorizationService:
    ...
    def validate_config_authorization_or_raise(
        ...
        violations = ConfigAuthorizationService._validate_elements(typed_elements, config, access_checker, t, prefix="")
        if violations:
            raise HTTPException(
                status_code=403,
                detail={
        ...
        for element in elements:
            ...
            if isinstance(element, Group):
                ...
                if isinstance(nested_config, dict):
                    violations.extend(
                        ConfigAuthorizationService._validate_elements(
                            element.children, nested_config, access_checker, t, prefix=f"{field_path}."
                        )
                    )
                ...
                if isinstance(items, list):
                    for i, item in enumerate(items):
                        if isinstance(item, dict):
                            violations.extend(
                                ConfigAuthorizationService._validate_elements(
                                    element.children, item, access_checker, t, prefix=f"{field_path}.{i}."
                                )
                            )
                ...
                if value is not None:
                    violations.extend(element.validate_authorization(field_path, value, access_checker, t))
