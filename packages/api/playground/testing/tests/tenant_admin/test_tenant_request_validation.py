...
class TestTenantIdConstraints:
    ...
    def test_accepts_any_non_empty_bounded_string(self, tenant_id: str) -> None:
        req = ConfigureTenantRequest.model_validate(_valid_configure_payload(tenant_id=tenant_id))
        assert req.tenant_id == tenant_id

    def test_rejects_empty_tenant_id(self) -> None:
        with pytest.raises(ValidationError):
            ConfigureTenantRequest.model_validate(_valid_configure_payload(tenant_id=""))

    def test_rejects_tenant_id_exceeding_255_chars(self) -> None:
        with pytest.raises(ValidationError):
            ConfigureTenantRequest.model_validate(_valid_configure_payload(tenant_id="a" * 256))


class TestTenantNameConstraints:
    def test_strips_surrounding_whitespace(self) -> None:
        req = ConfigureTenantRequest.model_validate(_valid_configure_payload(name="  My Tenant  "))
        assert req.name == "My Tenant"

    @pytest.mark.parametrize("name", ["", "   ", "\t\n"])
    def test_rejects_empty_or_whitespace_only_name(self, name: str) -> None:
        with pytest.raises(ValidationError):
            ConfigureTenantRequest.model_validate(_valid_configure_payload(name=name))

    def test_rejects_name_exceeding_120_chars(self) -> None:
        with pytest.raises(ValidationError):
            ConfigureTenantRequest.model_validate(_valid_configure_payload(name="x" * 121))

    def test_accepts_name_at_120_chars(self) -> None:
        req = ConfigureTenantRequest.model_validate(_valid_configure_payload(name="x" * 120))
        assert len(req.name) == 120


class TestTenantDescriptionConstraints:
    def test_accepts_empty_description(self) -> None:
        req = ConfigureTenantRequest.model_validate(_valid_configure_payload(description=""))
        assert req.description == ""

    def test_rejects_description_exceeding_500_chars(self) -> None:
        with pytest.raises(ValidationError):
            ConfigureTenantRequest.model_validate(_valid_configure_payload(description="x" * 501))


class TestUpdateTenantRequest:
    """Constraints on ``UpdateTenantRequest`` must mirror ``ConfigureTenantRequest``
    ...
    def test_rejects_whitespace_only_name(self) -> None:
        with pytest.raises(ValidationError):
            UpdateTenantRequest.model_validate({"name": "   "})

    def test_strips_whitespace_from_name_when_provided(self) -> None:
        req = UpdateTenantRequest.model_validate({"name": "  Renamed  "})
        assert req.name == "Renamed"

    def test_rejects_name_exceeding_120_chars(self) -> None:
        with pytest.raises(ValidationError):
            UpdateTenantRequest.model_validate({"name": "x" * 121})

    def test_rejects_description_exceeding_500_chars(self) -> None:
        with pytest.raises(ValidationError):
            UpdateTenantRequest.model_validate({"description": "x" * 501})

    def test_accepts_none_for_all_fields(self) -> None:
        req = UpdateTenantRequest.model_validate({"name": None, "description": None, "access_rules": None})
        assert req.name is None
