...
class TestLogSeverityFollowsWhoMustAct:
    """The span marks every one of these failures; the log level says which of them somebody is
...
class TestHandledFailureStaysVisibleInTracing:
    """Handling the exception is what made these requests invisible: the span no longer sees one.
    The 4xx case is the one that matters — the instrumentation derives UNSET from it afterwards."""
...
class TestRunnerRegistersTheHandler:
    """Registration lives in the runner, so every app it builds is covered without each
    controller repeating it — and losing it would silently restore the opaque 500."""
