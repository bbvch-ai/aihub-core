...
const { requestReport } = useIncidentReport()
...
// The action inside the iframe knows only the display, so the owning thread — the
// correct per-agent, salted one the pipe persisted events under — comes from the
// backend. Empty when no AI-Hub thread owns the display (e.g. a plain-LLM message).
...
const handleMessage = async (event: MessageEvent) => {
  ...
  // "Report a bug" on a single message: carry that message's model and thread into
  // the report. A plain-LLM message has no AI-Hub thread and is still worth
  // reporting, so an empty thread does not stop the form from opening.
  ...
  // Remember it, so a report raised from the rail still names the conversation.
...
onBeforeUnmount(() => {
  ...
  // The conversation is no longer on screen, so a report raised elsewhere in the
  // app must not claim it.
