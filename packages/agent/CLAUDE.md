...
| `AgentMemory`                            | User and organization memory access. Built from the profile's own extraction model via `AgentConfig.memory_llm_model_name` (issue #1590), falling back to `MEM0_LLM_NAME`                                                                                               |
...
| **MemoryWriterAgent**        | System agent: async user-memory persistence (issue #1179)    | Non-discoverable; triggered by `MemoryStorageRequestedEvent` from every RAG/ExpertRAG run that stores user memory — the only storage mode since ADR `2026_09_11_async_user_memory_storage_as_the_only_mode`, which is why those blueprints no longer announce `StoreUserMemoryEvent` and why the writer is a hard runtime dependency of theirs. Rebuilds the origin agent's `AgentMemory` and writes off the chat critical path. The origin's extraction model travels on the start event as `origin_memory_llm` (issue #1590) — the writer has no config of its own, so without it every delegated write would silently fall back to the platform default                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| **EmailClassificationAgent** | Non-conversational mailbox triage (issue #1637)              | Files every unread message into the folder for its category. Categories are a `MailCategory` repeater (name, folder, description) — the description is what the model classifies on. The model returns an *index* into the configured list, never a folder name, which is what contains prompt injection from untrusted mail. One route to the configured fallback folder: the model explicitly answering "none fit" (a self-reported confidence score was measured and removed — see the ADR). Filing is the dedup — every message leaves the inbox, so a re-run is a no-op and a half-failed batch is safe. One looping step (runtime-variable count), connection closed across the LLM calls. Never sends. **Schedulable** (issue #1638): the entry step accepts `ClassifyMailStartEvent \| CronStartEvent`, which is the whole opt-in — the `cron` field the scheduler reads lives on the `AgentConfig` base and is injected by `AgentRunner`, not declared here. Because scheduling makes overlapping runs reachable, a `MailboxRunLease` (`imap/mailbox_run_lease.py`, Redis `SET NX EX`; renew and release are Lua so the holder check and the write are one round trip) lets one run at a time hold a profile's mailbox — filing is only the dedup *between* runs, and mail stays unread until it moves. The work runs under `lease.heartbeat(...)`, which renews for as long as its body does anything, so the TTL bounds crash recovery rather than run length; per-phase renewal was not enough, since the batch fetch and the filing pass have no per-item hook. A lost lease raises `MailboxLeaseLostError` *before* filing — the only phase that mutates the mailbox. Released by whichever step returns the terminal `StopEvent`. **Drafts replies per category** (issue #1639): `MailCategory.draft_reply` opts a category in, and `draft_replies_step` (which replaced `finish_classification_step` — two steps on one event would both fire) appends a threaded draft per opted-in message; `finish_drafting_step` is the other terminal step. It reads each message back from the S3 archive rather than IMAP, because the UID died with the `MOVE` and a body has no business on a persisted event; filing, not a flag, is the dedup. Attachment text is optional (`include_attachments`), read through core's `DocumentExtractor`, which routes via `DocumentLoaderSelector` (PDF/image → MinerU, Office → MarkItDown, `.eml` → `EmlLoader`) and is asked `can_extract` first so an unreadable type costs no S3 fetch, floored at `min_attachment_bytes` so signature logos cost nothing, and a textless image is reported in the prompt's inventory rather than dropped. `DraftPromptBuilder` fits the prompt to `number_of_input_tokens`, sacrificing attachment extracts before trimming the body. **Bounded and isolated per message**: both prompts trim untrusted mail to a token budget (`TokenBudget` in `imap/token_budget.py`, shared by `DraftPromptBuilder` and `MailClassifier`) and cap the subject at `MAX_SUBJECT_CHARACTERS`, which lives in `packages/core/imap/mail_parser.py` beside the parser that enforces it, because the subject sits in the part neither trims. `MailClassifier.classify` and `_compose_one` never raise: a message that cannot be classified gets `ClassificationOutcome.FAILED` and is filed into `failure_folder`, and one that cannot be drafted is skipped. Both matter because filing is the only dedup — a raise before filing leaves the message unread, and `list_unread` re-selects it oldest-first on every run forever, blocking every message behind it. `_validate` now constructs the `DraftPromptBuilder` up front, so a budget too small to work fails before the batch is classified and filed rather than after. **Grounds drafts in the profile's knowledge agent** (issue #1720): configuring `knowledge_delegation` is what makes drafting grounded at all — `request_grounded_drafts_step` then fans out one `RAGStartEvent` per drafted message via `AgentInTheLoop`, and with no knowledge agent configured the whole batch keeps the local LLM path. What a category decides is only how far retrieval is narrowed: `MailCategory.knowledge_namespaces` is a nullable `KnowledgeCollectionSelector` whose options are the collections the chosen RAG agent is itself configured for (a collection outside that set is dropped by `narrow_retrievers` and answers from nothing, which is why the picker is sourced from the agent and not from the catalogue). Left off it sends no selection, which `narrow_retrievers` reads as the delegate's whole scope; turned on it narrows to the pairs it names, several per database included. `KnowledgeCollectionValidator` still fails the run up front on a collection the catalogue no longer holds. Both paths converge on one `MailBatchDraftedEvent` in `collect_and_draft_step` so `finish_drafting_step` stays the single terminal step. The join is the engine's runtime-variable one — a `list[AgentInTheLoop.response \| AgentInTheLoop.exception]` parameter plus a precondition counting *distinct* `request_event_id`s with `>=` (never `==`: at-least-once delivery can overshoot, and `FixedList` needs a compile-time N). Every outcome yields a draft — `RAGFailureStopEvent` gets `no_information_draft`, an AITL exception gets `grounding_failed_draft` — because filing is the only dedup and a message with no draft would sit in neither state. `share_run_id=False` is load-bearing: the response subscription is keyed by the delegated run id. `MailboxRunLease.reacquire` takes a lapsed-but-free lease back before appending, because the unbounded delegation window has no step body to heartbeat from and the mail is already filed by then. |
