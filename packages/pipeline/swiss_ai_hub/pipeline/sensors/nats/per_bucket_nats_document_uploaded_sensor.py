...
def per_bucket_nats_document_uploaded_sensor(
    ...
    *,
    ...
    @sensor(
        ...
        description="Polls each knowledge database it owns's JetStream for SourceUpdatedEvents.",
    )
    def _sensor(context: SensorEvaluationContext):
        async def check_for_events() -> list[RunRequest]:
            ...
            try:
                ...
                js = nc.jetstream()
                tick = context.last_tick_completion_time or 0
                ...
                logger.exception(f"Error in per-bucket NATS sensor: {e}")
                return []
