...
**Important**: The `datalake_directory_name` parameter determines the **namespace** used in the downstream document
ingestion pipeline (vector store). This affects how your data is organized and searchable.
...
will **not be processed** by the document ingestion pipeline. Only files within directories get a namespace and are
indexed.
