...
generic widgets), composables (auth, tenant, dashboard), plugins (`oidc-client`, `config-loader`, `apexcharts`), layouts
(`anonymous.vue`, `default.vue` from web), theme, tailwind config, primevue config, i18n setup, formkit config. Do NOT
duplicate any of those here.
...
sysadmin on every request; non-sysadmins are bounced to the main app at `${DOMAIN}/select-tenant` via the cross-origin
`mainApi.url` runtime config.
...
- `swiss-ai-hub-api` (main API, Apache-2.0) — cross-origin at `${DOMAIN}/api/v1/*`. Used for ONE call: `getMyTenants`
  ...
  middleware uses `$fetch` against `runtimeConfig.public.mainApi.url` directly.
...
- App entry: `.app/nuxt.config.ts` (runtimeConfig, dev proxy → :8001)
- SDK init: `app.vue` (sets `client.baseURL = '/api/v1'` + 403 handler)
- Sysadmin gate: `middleware/sysadmin.global.ts` (cross-origin `getMyTenants` check)
