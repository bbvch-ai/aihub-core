"""Testing-playground entry point — drives the bot from the Bot Framework Emulator WITHOUT Azure.

The ``microsoft-agents`` ``CloudAdapter`` always performs MSAL auth. Even to *receive* an activity
it builds a user-token client (needs a real ``TENANT_ID``), and to *reply* it signs the request with
a bearer token. A vanilla simulated server therefore fails with ``TENANT_ID is not set`` on inbound,
and a fake token gets rejected with ``401`` by the emulator on outbound.

This runner forces the SDK's built-in unauthenticated mode (``use_anonymous=True`` on the
channel-service factory) so no Azure/token is needed, binds ``0.0.0.0`` (reachable across the WSL2
boundary), and optionally stubs the user's email (``BOT_DEV_FAKE_EMAIL``) since the emulator can't do
the Teams member lookup. LOCAL TESTING USE ONLY — never an entry point for a deployed bot.
"""
