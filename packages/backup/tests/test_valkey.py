...
def tmp_dir(tmp_path: Path) -> Path:
    with patch("swiss_ai_hub.backup.services.valkey.tempfile.mkdtemp", return_value=str(tmp_path)):
        yield tmp_path
...
def test_backup_raises_on_bgsave_timeout(valkey_handler: ValkeyHandler, mock_redis: MagicMock) -> None:
    ...
    with (
        ...
        patch("swiss_ai_hub.backup.services.valkey.BGSAVE_TIMEOUT", 4),
        patch("swiss_ai_hub.backup.services.valkey.time.sleep"),
...
def test_restore_works_when_container_already_stopped(tmp_dir: Path, valkey_handler: ValkeyHandler) -> None:
    ...
    docker = valkey_handler._docker
    docker.stop_container.return_value = False
...
def test_restore_does_not_mutate_container_if_preparation_fails(tmp_dir: Path, valkey_handler: ValkeyHandler) -> None:
    ...
    with (
        patch("swiss_ai_hub.backup.services.valkey.shutil.copy2", side_effect=OSError("disk full")),
