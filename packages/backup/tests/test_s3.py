...
def s3_manager(settings: BackupSettings) -> S3Manager:
    with patch("swiss_ai_hub.backup.s3.boto3") as mock_boto3:
...
def test_resolve_timestamp_exact_match(s3_manager: S3Manager) -> None:
    ...
    s3_manager._client.get_paginator.return_value.paginate.return_value = [
        {"CommonPrefixes": [{"Prefix": "2026-02-18_14-00-00/"}]}
    ]
    result = s3_manager.resolve_timestamp("2026-02-18_14-00-00")
    assert result == "2026-02-18_14-00-00"
...
def test_find_latest_backup(s3_manager: S3Manager) -> None:
    s3_manager._client.get_paginator.return_value.paginate.return_value = [
        {
            "CommonPrefixes": [
                {"Prefix": "2026-02-17_02-00-00/"},
                {"Prefix": "2026-02-18_02-00-00/"},
            ]
        }
    ]
    result = s3_manager.find_latest_backup()
    assert result == "2026-02-18_02-00-00"
...
def test_delete_recursive(s3_manager: S3Manager) -> None:
    ...
    s3_manager._client.delete_objects.return_value = {"Deleted": [{"Key": "prefix/a"}, {"Key": "prefix/b"}]}
...
def test_list_backups(s3_manager: S3Manager) -> None:
    s3_manager._client.get_paginator.return_value.paginate.side_effect = [
        # list_prefixes call
        [
            {
                "CommonPrefixes": [
        ...
        # count_objects for first prefix
        [{"KeyCount": 5}],
        # count_objects for second prefix
        [{"KeyCount": 3}],
    ]

    entries = s3_manager.list_backups()

    assert len(entries) == 2
    assert entries[0].prefix == "2026-02-17_02-00-00"
    assert entries[1].prefix == "2026-02-18_02-00-00"
    assert entries[0].file_count == 5
    assert entries[1].file_count == 3


def test_list_backups_skips_non_backup_prefixes(s3_manager: S3Manager) -> None:
    s3_manager._client.get_paginator.return_value.paginate.side_effect = [
        # list_prefixes call
        [
            {
        ...
        # count_objects for matching prefix only
        [{"KeyCount": 5}],
    ]
...
def test_delete_recursive_raises_on_partial_failure(s3_manager: S3Manager) -> None:
    s3_manager._client.get_paginator.return_value.paginate.return_value = [
        {"Contents": [{"Key": "prefix/a"}, {"Key": "prefix/b"}]}
    ]
    s3_manager._client.delete_objects.return_value = {
        "Deleted": [{"Key": "prefix/a"}],
        "Errors": [{"Key": "prefix/b", "Code": "AccessDenied", "Message": "forbidden"}],
    }
