...
def _manager(pipeline_type: str = "document_ingestion") -> PipelineTypeTopicManager:
...
class TestStream:
    def test_one_stream_and_filter_per_ingestor_regardless_of_database_count(self):
        assert _manager().get_stream() == ("pipeline_document_ingestion_stream", "pipeline.document_ingestion.>")
    ...
    def test_two_ingestors_get_non_overlapping_stream_filters(self):
        ...
        _, platform_filter = _manager().get_stream()
        _, acme_filter = _manager("acme_rag").get_stream()

        assert platform_filter != acme_filter
        assert not acme_filter.startswith(platform_filter.removesuffix(">"))
...
class TestSubject:
    def test_the_bucket_survives_a_round_trip_through_the_subject(self):
        ...
        assert subject == (
            "pipeline.document_ingestion.researchdocs.to.knowledge.researchdocs.rk.SourceUpdatedEvent.eid"
        )
