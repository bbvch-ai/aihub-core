...
def mock_database_operations(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        "swiss_ai_hub.core.persistence.user.UserEntity.UserEntity.ensure_user_exists_for_auth", mock_ensure_user_exists
    )
    monkeypatch.setattr(
        "swiss_ai_hub.core.auth.identity.UserIdentity.UserIdentity.from_user_entity", mock_from_user_entity
    )
