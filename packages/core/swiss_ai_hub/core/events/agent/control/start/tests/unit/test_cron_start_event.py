...
class TestCronStartEvent:
    def test_starts_a_run(self):
        assert issubclass(CronStartEvent, StartEvent)
        assert CronStartEvent(scheduled_for=_OCCURRENCE).is_start_event
    ...
    def test_is_not_a_user_message(self):
        ...
        assert not issubclass(CronStartEvent, UserMessageEvent)
    ...
    def test_carries_no_user_by_default(self):
        ...
        assert CronStartEvent(scheduled_for=_OCCURRENCE).user is None

    def test_carries_the_occurrence_it_fired_for(self):
        assert CronStartEvent(scheduled_for=_OCCURRENCE).scheduled_for == _OCCURRENCE
    ...
    def test_polymorphic_round_trip(self):
        event = CronStartEvent(scheduled_for=_OCCURRENCE)
        restored = BaseEvent.deserialize_event(event.model_dump())

        assert isinstance(restored, CronStartEvent)
    ...
    def test_context_dict_is_json_serializable(self):
        ...
        """
        context = CronStartEvent(scheduled_for=_OCCURRENCE).to_context_dict()
    ...
    def test_occurrence_survives_the_context_round_trip(self):
        ...
        context = json.loads(json.dumps(CronStartEvent(scheduled_for=_OCCURRENCE).to_context_dict()))
    ...
    def test_display_name_and_description_resolve(self, locale: str):
        assert CronStartEvent._display_name.in_locale(locale)
        assert CronStartEvent._display_description.in_locale(locale)
