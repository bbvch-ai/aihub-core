...
class TestNCRequester:
    ...
    async def test_request_success(
        ...
        # Make request
        with patch("swiss_ai_hub.core.nats.requester.NCRequester.get_tracer") as mock_tracer:
            mock_span = MagicMock()
...
class TestNCResponder:
    ...
    async def test_process_request_calls_handler(
        ...
        with patch("swiss_ai_hub.core.nats.responder.NCResponder.get_tracer") as mock_tracer:
            mock_span = MagicMock()
            mock_span.__enter__ = MagicMock(return_value=mock_span)
            mock_span.__exit__ = MagicMock(return_value=None)
            mock_tracer.return_value.start_as_current_span.return_value = mock_span
