...
SDK types generated              → packages/web/aihub_web/sdk/client/ (pnpm generate-sdk)
  ↓ imported by composables
Frontend composables + pages     → packages/web/aihub_web/composables/agent/
```
...
ProcessEndpointsDiscoveryService → packages/api/swiss_ai_hub/api/services/ProcessEndpointsDiscoveryService.py
  ↓ creates dynamic API routes
SDK types + composables          → packages/web/aihub_web/composables/process/
```
...
useThreadEvents composable       → packages/web/aihub_web/composables/thread/useThreadEvents.ts
  ↓ routes to component
useEventComponent resolver       → packages/web/aihub_web/composables/event/useEventComponent.ts
  ↓ maps event._event_name → Vue component
Event display component          → packages/web/aihub_web/components/Event/Display/{EventName}.vue
```
...
buildFormKitSchema()             → packages/web/aihub_web/composables/form/useFormKitTransform.ts
...
1. Each custom FormkitElement type used (e.g., `ModelSelect`, `LocaleInput`, `IconPicker`) has a corresponding
   registration in `packages/web/aihub_web/.app/formkit.config.ts`
...
| `packages/web`     | `packages/web/aihub_web/i18n/locales/`                             | (flat)     |
...
Grep "{EventName}" in packages/web/aihub_web/composables/event/useEventComponent.ts
Glob "packages/web/aihub_web/components/Event/Display/{EventName}.vue"

# Chain 4: Check FormKit custom input registration
Grep "{elementType}" in packages/web/aihub_web/.app/formkit.config.ts
