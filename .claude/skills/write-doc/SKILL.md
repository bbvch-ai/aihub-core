---
name: write-doc
description: Write new documentation pages for the VitePress docs site at aihub_doc/docs/. Ensures correct file structure (index.en.md only), frontmatter, heading hierarchy, media references, VitePress containers, and adherence to writing guidelines. Use when user says "write documentation", "create a doc page", "add documentation for X", "document this feature", "new docs page", or "write a guide for X". Do NOT use for ADR creation (use document-decision), feature deep-dives from code analysis (use document-feature), or editing solution/tender documents (use document-solution).
...
1. Read the docs scope guide: `aihub_doc/CLAUDE.md`
2. Study 2-3 existing pages near where the new page will live:
   - Platform example: `aihub_doc/docs/2_platform/5_agents/index.en.md`
   - SDK example: `aihub_doc/docs/3_sdk/2_building_agents/1_agent_fundamentals/index.en.md`
   - Ecosystem example: `aihub_doc/docs/4_ecosystem/2_using_ai/3_vibe_coding_guidelines/index.en.md`
...
- The title appears in the auto-generated sidebar (`aihub_doc/.vitepress/sidebar-logic.mjs`)
...
Store in `aihub_doc/media/` organized by topic. Reference with relative paths:
...
1. Check frontmatter has `title`:
   ```bash
   head -5 aihub_doc/docs/<section>/<N>_<topic>/index.en.md
   ```
2. Preview the page:
   ```bash
...
6. Run translation to generate German version:
   ```bash
   cd aihub_doc && pnpm run docs:translate
   ```
