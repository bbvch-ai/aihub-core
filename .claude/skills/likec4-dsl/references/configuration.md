...
LikeC4 projects are defined by a config file. The file's location determines project scope — all `.c4` files in the
directory (and subdirectories) belong to that project.
...
Auto-derive `technology` from icon name when not set explicitly. Applies to `aws:`, `azure:`, `gcp:`, `tech:` icons.
Default: `true`.
...
Each config file in the workspace defines a separate project. Files belong to the project of the nearest config file in
...
**Important:** Always include `"$schema"` (recommended) — it enables IDE autocomplete and validation for your config
file.
