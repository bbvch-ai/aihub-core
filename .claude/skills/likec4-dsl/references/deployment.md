...
The `deployment` block maps logical model elements to physical infrastructure nodes using `instanceOf`. It uses
`deploymentNode` kinds from the specification.
...
- Use named instances (`id = instanceOf ELEMENT`) when multiple instances of the same element exist within the same
  deployment node.
