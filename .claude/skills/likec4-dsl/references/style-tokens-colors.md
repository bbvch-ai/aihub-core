...
LikeC4 provides a curated set of semantic color tokens. **Prefer these named tokens over raw hex colors** for
consistency and automatic light/dark theme adaptation.
...
Icons use the format `group:name` (e.g. `tech:react`, `aws:lambda`, `azure:app-service`). Five built-in groups are
available:
...
**Best practice:** Use semantic color tokens for all element and relationship coloring. Define custom named colors in
specification only for brand compliance, never use raw hex in styles.
