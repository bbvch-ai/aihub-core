...
The `specification` block defines all named vocabularies for the project: element kinds, deployment node kinds,
relationship kinds, tags, and custom color tokens. Every kind used in `model` or `deployment` blocks must be declared
here first.
...
- Keep specification in a dedicated file, e.g. `specification.c4` — changes to specification trigger a full project
  re-parse, so isolating it reduces edit latency on model/view files.
