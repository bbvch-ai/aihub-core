...
**Exception:** If the prompt explicitly asks for bidirectionality or if the system model emphasizes that *both elements
drive interaction*, use `<->`.
...
**When in doubt:** use `->` for request-response patterns (REST, async jobs, events). Reserve `<->` for explicitly
symmetric interactions documented in requirements.
