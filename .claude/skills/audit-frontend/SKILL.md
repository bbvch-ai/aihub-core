...
Run a comprehensive audit of the Nuxt 3 admin interface at `packages/web/swiss_ai_hub_web/`. Scope via `$ARGUMENTS`:
`all`, `composables`, `typescript`, `components`, `pages`, `sdk`, `i18n`, `tailwind`.
...
Check all files in `packages/web/swiss_ai_hub_web/composables/` against these rules:
...
grep -rn "export.*=.*(" packages/web/swiss_ai_hub_web/composables/ --include="*.ts" | grep -v defineQuery | grep -v defineMutation
```
...
grep -rn "key: \[" packages/web/swiss_ai_hub_web/composables/ --include="*.ts"
```
...
grep -rn "useQuery" packages/web/swiss_ai_hub_web/composables/ --include="*.ts" -A 5 | grep -v staleTime
```
...
grep -rn "await \(get\|create\|update\|delete\|post\|put\|patch\)" packages/web/swiss_ai_hub_web/composables/ --include="*.ts"
```
...
Reference implementation: `packages/web/swiss_ai_hub_web/composables/useRouteReady.ts`
...
grep -rn "export default" packages/web/swiss_ai_hub_web/composables/ --include="*.ts"
```
...
grep -rn "=> {" packages/web/swiss_ai_hub_web/pages/ packages/web/swiss_ai_hub_web/components/ --include="*.vue" --include="*.ts" | grep "(event)" | grep -v ": "
```
...
grep -rn "import {.*Dto" packages/web/swiss_ai_hub_web/ --include="*.vue" --include="*.ts" | grep -v "import type"
```
...
```bash
grep -rn "console\.\(log\|debug\|info\)" packages/web/swiss_ai_hub_web/ --include="*.vue" --include="*.ts" \
...
find packages/web/swiss_ai_hub_web/components/ -name "*.vue" | while read f; do
  # Agent/Card.vue → AgentCard
  NAME=$(echo "$f" | sed 's|.*/components/||; s|\.vue$||; s|/||g')
  USAGE=$(grep -rl "$NAME" packages/web/swiss_ai_hub_web/ --include="*.vue" --include="*.ts" | grep -v "$f" | wc -l)
...
grep -rn "bg-surface-\(50\|100\|200\)" packages/web/swiss_ai_hub_web/components/ --include="*.vue" \
...
```bash
grep -rL "StructuralScreen" packages/web/swiss_ai_hub_web/pages/service/*.vue
```
...
grep -rn "router\.push\|navigateTo" packages/web/swiss_ai_hub_web/pages/ packages/web/swiss_ai_hub_web/components/ --include="*.vue" \
...
grep -rn "import.*from 'vue'" packages/web/swiss_ai_hub_web/pages/ packages/web/swiss_ai_hub_web/components/ --include="*.vue"
grep -rn "import.*from 'vue-router'" packages/web/swiss_ai_hub_web/pages/ --include="*.vue"
grep -rn "import.*from 'vue-i18n'" packages/web/swiss_ai_hub_web/pages/ --include="*.vue"
grep -rn "import.*from 'primevue/use" packages/web/swiss_ai_hub_web/pages/ --include="*.vue"
```
...
```bash
stat -c %Y packages/web/swiss_ai_hub_web/sdk/client/sdk.gen.ts
...
1. Read all locale files: `packages/web/swiss_ai_hub_web/i18n/locales/{de,en,fr,it}.yaml`
...
grep -rn "<style" packages/web/swiss_ai_hub_web/components/ packages/web/swiss_ai_hub_web/pages/ --include="*.vue"
```

### 7b. Inline styles (WARNING)

```bash
grep -rn 'style="' packages/web/swiss_ai_hub_web/components/ packages/web/swiss_ai_hub_web/pages/ --include="*.vue" | grep -v ":style"
```
...
```bash
grep -rn "text-\(gray\|slate\|zinc\|neutral\|stone\)-" packages/web/swiss_ai_hub_web/ --include="*.vue"
grep -rn "bg-\(gray\|slate\|zinc\|neutral\|stone\)-" packages/web/swiss_ai_hub_web/ --include="*.vue"
```
