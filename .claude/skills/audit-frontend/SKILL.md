...
Run a comprehensive audit of the Nuxt 3 admin interface at `packages/web/`. Scope via `$ARGUMENTS`: `all`,
...
Check all files in `packages/web/composables/` against these rules:
...
grep -rn "export.*=.*(" packages/web/composables/ --include="*.ts" | grep -v defineQuery | grep -v defineMutation
```
...
grep -rn "key: \[" packages/web/composables/ --include="*.ts"
```
...
grep -rn "useQuery" packages/web/composables/ --include="*.ts" -A 5 | grep -v staleTime
```
...
grep -rn "await \(get\|create\|update\|delete\|post\|put\|patch\)" packages/web/composables/ --include="*.ts"
```
...
Reference implementation: `packages/web/composables/useRouteReady.ts`
...
grep -rn "export default" packages/web/composables/ --include="*.ts"
```
...
grep -rn "=> {" packages/web/pages/ packages/web/components/ --include="*.vue" --include="*.ts" | grep "(event)" | grep -v ": "
```
...
grep -rn "import {.*Dto" packages/web/ --include="*.vue" --include="*.ts" | grep -v "import type"
```
...
```bash
grep -rn "console\.\(log\|debug\|info\)" packages/web/ --include="*.vue" --include="*.ts" \
...
find packages/web/components/ -name "*.vue" | while read f; do
  # Agent/Card.vue → AgentCard
  NAME=$(echo "$f" | sed 's|.*/components/||; s|\.vue$||; s|/||g')
  USAGE=$(grep -rl "$NAME" packages/web/ --include="*.vue" --include="*.ts" | grep -v "$f" | wc -l)
...
grep -rn "bg-surface-\(50\|100\|200\)" packages/web/components/ --include="*.vue" \
...
grep -rn "import.*from 'vue'" packages/web/pages/ packages/web/components/ --include="*.vue"
grep -rn "import.*from 'vue-router'" packages/web/pages/ --include="*.vue"
grep -rn "import.*from 'vue-i18n'" packages/web/pages/ --include="*.vue"
...
2. If accessible, compare OpenAPI spec endpoints with `packages/web/sdk/client/sdk.gen.ts`:
...
```bash
stat -c %Y packages/web/sdk/client/sdk.gen.ts
...
1. Read all locale files: `packages/web/i18n/locales/{de,en,fr,it}.yaml`
...
grep -rn "<style" packages/web/components/ packages/web/pages/ --include="*.vue"
```

### 7b. Inline styles (WARNING)

```bash
grep -rn 'style="' packages/web/components/ packages/web/pages/ --include="*.vue" | grep -v ":style"
```
...
```bash
grep -rn "text-\(gray\|slate\|zinc\|neutral\|stone\)-" packages/web/ --include="*.vue"
grep -rn "bg-\(gray\|slate\|zinc\|neutral\|stone\)-" packages/web/ --include="*.vue"
```
