...
class Neo4jHandler(BackupHandler):
    ...
    which requires exclusive access to the ``/data`` directory — it cannot run
    while the Neo4j process holds a lock on it.  The main container is stopped
    globally by ``backup_session`` / ``restore_session``, but a stopped
    container cannot execute commands (``docker exec`` needs a running process).

    The workaround is a *sibling container*: a short-lived container created
    from the **same Neo4j image** with the **same /data volume** mounted.
    Both image tag and volume source are introspected from the production
    container at runtime, so no versions or paths are hardcoded.

    Lifecycle (backup):
        1. Create sibling container → run ``neo4j-admin database dump``
        2. Copy dump file out of the stopped sibling → upload to S3
        3. Remove sibling container (always, via ``finally``)

    Lifecycle (restore):
        1. Download dump from S3 → copy into a new sibling container
        2. Start sibling → run ``neo4j-admin database load``
        3. Remove sibling container (always, via ``finally``)
    """
