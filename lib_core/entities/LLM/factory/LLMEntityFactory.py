...
from lib_core.entities.LLM.embedding.AzureOpenAIEmbeddingEntity import AzureOpenAIEmbeddingEntity
from lib_core.entities.LLM.embedding.SelfHostedEmbeddingEntity import SelfHostedEmbeddingEntity
...
class LLMEntityFactory:
    ...
    def by_names(organization_shortname: str, names: List[str]) -> List[LLMEntityType]:
        return list(LLMEntity.objects.using(organization_shortname).filter(name__in=names).order_by("context_size"))

    @staticmethod
    def all(organization_shortname: str) -> List[LLMEntityType]:
        return list(LLMEntity.objects.using(organization_shortname).order_by("context_size"))
