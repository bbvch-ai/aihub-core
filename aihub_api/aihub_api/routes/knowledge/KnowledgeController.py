...
class KnowledgeController(Controller):
    ...
    def __init__(self, vector_store_factory: VectorStoreFactory, route: str = "/knowledge", auth: AuthHandler | None = None, is_admin_only=True):
        super().__init__(route, auth, is_admin_only=is_admin_only)
        self.docstore_client: MongoClient = connect(host=CosmosDocstoreAccess().get_connection_string(), alias='docstore')
    ...
    def get_documents_for_namespace(self, route: str = "/db/{db}/namespace/{namespace}/document") -> "KnowledgeController":
        @self.router.get(route, tags=self.tags)
    ...
    def get_document_by_id(self, route: str = "/db/{db}/namespace/{namespace}/document/{document_id}") -> "KnowledgeController":
        @self.router.get(route, tags=self.tags)
    ...
    def get_nodes_for_document(self, route: str = "/db/{db}/namespace/{namespace}/document/{document_id}/nodes") -> "KnowledgeController":
        @self.router.get(route, tags=self.tags)
        async def get_nodes_for_document(
            ...
            if db in ['admin', 'local', 'config']:
                raise HTTPException(status_code=403, detail="Not authorized to view this database")
            return KnowledgeService.get_nodes(db=db, namespace=namespace, document_id=document_id, vector_store_factory=self.vector_store_factory)

        return self

    def get_summary_nodes_for_document(self, route: str = "/db/{db}/namespace/{namespace}/document/{document_id}/summaries") -> "KnowledgeController":
        @self.router.get(route, tags=self.tags)
        async def get_summary_nodes_for_document(
            ...
            if db in ['admin', 'local', 'config']:
                raise HTTPException(status_code=403, detail="Not authorized to view this database")
            return KnowledgeService.get_summary_nodes(db=db, namespace=namespace, document_id=document_id, vector_store_factory=self.vector_store_factory)
