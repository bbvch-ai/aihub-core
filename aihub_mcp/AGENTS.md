...
- **Authentication**: API key is required when `MCP_REQUIRE_AUTH=true` (default). Set `MCP_REQUIRE_AUTH=false` to disable.
- **Network Access**: Default host is `127.0.0.1` (localhost only). Set to `0.0.0.0` for external access with authentication.
