...
from playground.minimal_workflow.ContextAgent.Events.CustomStartEvent import (
    CustomStartEvent,
)
...
class ContextAgent(Agent):
    ...
    async def start_step(
        ...
        print(
            f"[SimpleAgent.start_step] Called {thread_count} times in thread, {run_count} times in run"
        )
    ...
    async def end_step(
        ...
        run_count = await run_context.get("count", 0)
        print(
            f"[SimpleAgent.end_step] Called {thread_count} times in thread, {run_count} times in run"
        )
        return StopEvent()
