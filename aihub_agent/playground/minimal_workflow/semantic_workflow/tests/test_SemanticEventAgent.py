...
def test_retrieve_present(agent_runner: AgentTestRunner):
    assert agent_runner.has_start_event, "Agent did not receive retriever event"


@then("a RerankerEvent is present")
def test_rerank_present(agent_runner: AgentTestRunner):
    assert agent_runner.has_start_event, "Agent did not receive reranker event"


@then("a LLMStopEvent is present")
def test_retrieve_present(agent_runner: AgentTestRunner):
    assert agent_runner.has_stop_event, "Agent did not stop start event"
