...
You are contributing to the **aihub\_agent** scope, which contains all agent logic and workflow definitions within the
...
2. **Define the Agent Configuration**: Create a Pydantic model inheriting from `AgentConfig` to hold the agent's
   ...
   class MyAgentConfig(AgentConfig):
       temperature: Annotated[float, Field(description="LLM temperature", ge=0.0, le=1.0)] = 0.7
       confidence_threshold: Annotated[float, Field(description="Minimum confidence threshold")] = 0.5
   ```
...
::: tip Debugging Tools
Your primary tools are **Phoenix Tracing** for visual flow analysis and **structured logging** for detailed event
...
**Phoenix** is your most important debugging tool. It provides a web UI to visualize the step-by-step execution of your
...
The Phoenix MCP server is running alongside the development environment and provides programmatic access to trace data
for debugging and monitoring agent executions. This integration allows you to query trace information directly from your
development tools.
:::
