...
class RunEventStore:
    ...
    def get_events_of_type(self, event_type: str, until: Optional[int] = None) -> List[ControlEvent]:
    ...
    def get_events_of_multiple_types(
        self, event_types: List[str], until: Optional[int] = None
    ) -> Dict[str, List[ControlEvent]]:
        """Get events of multiple types, organized by type name"""
        result = {}
        for event_type in event_types:
            result[event_type] = self.get_events_of_type(event_type, until)
        return result
