...
aspects of their jobs while humans maintain oversight for key decisions.
:::
...
Use the following commands to manage dependencies within an activated scope environment. Do not edit the
`pyproject.toml`
...
* **Require a Pull Request Before Merging**: All changes must be made through a pull request. Direct pushes to `main`
  are
  blocked.
...
* **Block Force Pushes and Deletions**: Force pushing to `main` is denied to preserve commit history. Deleting the
  `main`
...
::: tip :tag: Versioning and Referencing
